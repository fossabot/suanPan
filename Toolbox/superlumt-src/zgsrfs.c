...
void zgsrfs(trans_t trans, SuperMatrix* A, SuperMatrix* L, SuperMatrix* U,
	...
	for(j = 0; j < nrhs; ++j) {
	    ...
	    CCOPY(&nrow, Bptr, &ione, work, &ione);
			...
			if(notran) {
				for(k = 0; k < A->ncol; ++k) {
					xk = z_abs1(&Xptr[k]);
					for(i = Astore->colptr[k]; i < Astore->colptr[k + 1]; ++i) rwork[Astore->rowind[i]] += z_abs1(&Aval[i]) * xk;
				}
			} else {
				for(k = 0; k < A->ncol; ++k) {
					s = 0.;
			...
			s = 0.;
			for(i = 0; i < A->nrow; ++i) {
				if(rwork[i] > safe2) { s = SUPERLU_MAX(s, z_abs1(&work[i]) / rwork[i]); } else if(rwork[i] != 0.0) { s = SUPERLU_MAX(s, (z_abs1(&work[i]) + safe1) / rwork[i]); }
		...
		for(i = 0; i < A->nrow; ++i)
			if(rwork[i] > safe2) rwork[i] = z_abs(&work[i]) + (iwork[i] + 1) * eps * rwork[i];
			else rwork[i] = z_abs(&work[i]) + (iwork[i] + 1) * eps * rwork[i] + safe1;
		kase = 0;
		...
		do {
			...
			if(kase == 1) {
				...
				for(i = 0; i < A->nrow; ++i) { zd_mult(&work[i], &work[i], rwork[i]); }
			} else {
		...
		/* Normalize error. */
		lstres = 0.;
		if(notran && colequ) { for(i = 0; i < A->nrow; ++i) lstres = SUPERLU_MAX(lstres, C[i] * z_abs1( &Xptr[i])); } else if(!notran && rowequ) { for(i = 0; i < A->nrow; ++i) lstres = SUPERLU_MAX(lstres, R[i] * z_abs1( &Xptr[i])); } else { for(i = 0; i < A->nrow; ++i) lstres = SUPERLU_MAX(lstres, z_abs1( &Xptr[i])); }
