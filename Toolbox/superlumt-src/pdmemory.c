 ...
 */
float pdgstrf_MemInit(int_t n, int_t annz, superlumt_options_t* superlumt_options,
	...
	if(!dexpanders) dexpanders = (ExpHeader*)SUPERLU_MALLOC(NO_MEMTYPE * sizeof(ExpHeader));
	...
	if(refact == NO) {
		...
		} else { pdgstrf_SetupSpace(work, lwork); }
	    ...
	    printf(".. pdgstrf_MemInit(): #retries " IFMT "\n", ++retries);
			...
			if(whichspace == SYSTEM) {
				...
				SUPERLU_FREE(lsub);
				SUPERLU_FREE(usub);
			} else { duser_free(nzumax * dword + (nzlmax + nzumax) * iword, HEAD); }
...
int_t pdgstrf_WorkInit(int_t n, int_t panel_size, int_t** iworkptr, double** dworkptr) {
	...
	else *iworkptr = (int_t*)duser_malloc(isize, TAIL);
	if(! *iworkptr) {
		fprintf(stderr, "pdgstrf_WorkInit: malloc fails for local iworkptr[]\n");
		return (isize + n);
	}

	if(whichspace == SYSTEM) *dworkptr = (double*)SUPERLU_MALLOC((size_t) dsize);
	else {
 ...
 */
void pdgstrf_SetRWork(int_t n, int_t panel_size, double* dworkptr,
...
int_t pdgstrf_MemXpand(
	...
	if(mem_type == USUB) new_mem = pdgstrf_expand(maxlen, mem_type, next, 1, Glu);
	else new_mem = pdgstrf_expand(maxlen, mem_type, next, 0, Glu);
...
void copy_mem_double(int_t howmany, void* old, void* new) {
	register int_t i;
	double* dold = old;
	double* dnew = new;
...
* pdgstrf_expand(
	...
	if(whichspace == SYSTEM) {
		...
		if(no_expand != 0) {
			...
			if(type == LSUB || type == USUB) { copy_mem_int(len_to_copy, dexpanders[type].mem, new_mem); } else { copy_mem_double(len_to_copy, dexpanders[type].mem, new_mem); }
