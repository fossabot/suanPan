 ...
 */
float pcgstrf_MemInit(int_t n, int_t annz, superlumt_options_t* superlumt_options,
	...
	if(!cexpanders) cexpanders = (ExpHeader*)SUPERLU_MALLOC(NO_MEMTYPE * sizeof(ExpHeader));
	...
	if(refact == NO) {
		...
		} else { pcgstrf_SetupSpace(work, lwork); }
	    ...
	    printf(".. pcgstrf_MemInit(): #retries " IFMT "\n", ++retries);
			...
			if(whichspace == SYSTEM) {
				...
				SUPERLU_FREE(lsub);
				SUPERLU_FREE(usub);
			} else { cuser_free(nzumax * dword + (nzlmax + nzumax) * iword, HEAD); }
 ...
 */
int_t pcgstrf_WorkInit(int_t n, int_t panel_size, int_t** iworkptr, complex** dworkptr) {
	...
	else *iworkptr = (int_t*)cuser_malloc(isize, TAIL);
	if(! *iworkptr) {
		fprintf(stderr, "pcgstrf_WorkInit: malloc fails for local iworkptr[]\n");
		return (isize + n);
	}

	if(whichspace == SYSTEM) *dworkptr = (complex*)SUPERLU_MALLOC((size_t) dsize);
	else {
 ...
 */
void pcgstrf_SetRWork(int_t n, int_t panel_size, complex* dworkptr,
...
int_t pcgstrf_MemXpand(
	...
	if(mem_type == USUB) new_mem = pcgstrf_expand(maxlen, mem_type, next, 1, Glu);
	else new_mem = pcgstrf_expand(maxlen, mem_type, next, 0, Glu);
...
void copy_mem_complex(int_t howmany, void* old, void* new) {
	register int_t i;
	complex* dold = old;
	complex* dnew = new;
...
* pcgstrf_expand(
	...
	if(whichspace == SYSTEM) {
		...
		if(no_expand != 0) {
			...
			if(type == LSUB || type == USUB) { copy_mem_int(len_to_copy, cexpanders[type].mem, new_mem); } else { copy_mem_complex(len_to_copy, cexpanders[type].mem, new_mem); }
 ...
 */
void pcgstrf_StackCompress(GlobalLU_t* Glu) {
 ...
 */
void callocateA(int_t n, int_t nnz, complex** a, int_t** asub, int_t** xa) {
