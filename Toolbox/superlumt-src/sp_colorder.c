...
void sp_colorder(SuperMatrix* A, int_t* perm_c, superlumt_options_t* options,
                 SuperMatrix* AC) {
	/*
	...
	if(refact == NO) {
		...
		if(options->SymmetricMode) {
			...
			for(j = 0; j < n; ++j) {
				for(i = c_colbeg[j]; i < c_colend[j]; ++i) { b_rowind[i] = perm_c[b_rowind[i]]; }
			...
			for(i = 0; i < bnz; ++i) b_rowind[i] = iwork[b_rowind[i]];
	...
	print_int_vec("postorder etree:", n, options->etree);
		...
		for(i = 0; i < n; ++i) iwork[i] = post[perm_c[i]]; /* product of perm_c and post */
...
int_t dCheckZeroDiagonal(int_t n, int_t* rowind, int_t* colbeg,
...
int_t dPrintSuperPart(char* pname, int_t n, int_t* part_super) {
	...
	strcat(fname, ".dat");
	fp = fopen(fname, "w");
	for(i = 0; i < n; ++i) if(part_super[i]) fprintf(fp, IFMT, i);
	fprintf(fp, IFMT, n);
	fclose(fp);
	return 0;
}
