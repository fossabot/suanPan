...
void pzgstrf_thread_finalize(pzgstrf_threadarg_t* pzgstrf_threadarg,
	 ...
	 * =======
	 * 
	 * pzgstrf_thread_finalize() performs cleanups after the multithreaded 
	 * factorization pzgstrf_thread(). It sets up the L and U data
	 * structures, and deallocats the storage associated with the structures
	 * pxgstrf_shared and pzgstrf_threadarg.
	 *
	 ...
	 * =========
	 *
	 * pzgstrf_threadarg (input) pzgstrf_threadarg_t*
	...
	GlobalLU_t* Glu;
	extern ExpHeader* zexpanders;

	n = A->ncol;
	superlumt_options = pzgstrf_threadarg->superlumt_options;
	...
	if(superlumt_options->refact == YES) {
	   ...
	   pivoting, although the storage is available. */
		...
		zCreate_SuperNode_Permuted(L, A->nrow, A->ncol, nnzL, Glu->lusup,
		                           Glu->xlusup, Glu->xlusup_end,
		                           Glu->lsub, Glu->xlsub, Glu->xlsub_end,
		                           Glu->supno, Glu->xsup, Glu->xsup_end,
		                           SLU_SCP, SLU_Z, SLU_TRLU);
		zCreate_CompCol_Permuted(U, A->nrow, A->ncol, nnzU, Glu->ucol,
		                         Glu->usub, Glu->xusub, Glu->xusub_end,
		                         SLU_NCP, SLU_Z, SLU_TRU);
	}
	...
	for(i = 0; i < nprocs; ++i) {
		if(pzgstrf_threadarg[i].info) {
			if(iinfo) iinfo = SUPERLU_MIN(iinfo, pzgstrf_threadarg[i].info);
			else iinfo = pzgstrf_threadarg[i].info;
		}
	}
	*pxgstrf_shared->info = iinfo;
	...
	SUPERLU_FREE(pzgstrf_threadarg);
	...
	SUPERLU_FREE(zexpanders);
	zexpanders = 0;

#if ( DEBUGlevel>=1 )
    printf("** pzgstrf_thread_finalize() called\n");
#endif
}
