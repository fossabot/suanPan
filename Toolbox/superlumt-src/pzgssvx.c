...
void pzgssvx(int_t nprocs, superlumt_options_t* superlumt_options, SuperMatrix* A,
	...
	*info = 0;
	...
	/* ------------------------------------------------------------
	   ...
	   ------------------------------------------------------------*/
	if(notran) { if(rowequ) { for(j = 0; j < nrhs; ++j) for(i = 0; i < A->nrow; ++i) { zd_mult(&Bmat[i+j*ldb], &Bmat[i+j*ldb], R[i]); } } } else if(colequ) { for(j = 0; j < nrhs; ++j) for(i = 0; i < A->nrow; ++i) { zd_mult(&Bmat[i+j*ldb], &Bmat[i+j*ldb], C[i]); } }
	...
	if(*info > 0) {
		...
		*recip_pivot_growth = zPivotGrowth(A->ncol, AA, perm_c, L, U);
		...
		/* ------------------------------------------------------------
		   ...
		   system.
		   ------------------------------------------------------------*/
		if(notran) { if(colequ) { for(j = 0; j < nrhs; ++j) for(i = 0; i < A->nrow; ++i) { zd_mult(&Xmat[i+j*ldx], &Xmat[i+j*ldx], C[i]); } } } else if(rowequ) { for(j = 0; j < nrhs; ++j) for(i = 0; i < A->nrow; ++i) { zd_mult(&Xmat[i+j*ldx], &Xmat[i+j*ldx], R[i]); } }
