...
* pcgstrf_thread(void* arg) {
	 ...
	 *
	 *	THERE ARE 3 OF THEM:
	 *          marker[0 : m-1]:   used by pcgstrf_factor_snode() and 
	 *                                     pcgstrf_panel_dfs();
	 *          marker[m : 2m-1]:  used by pcgstrf_panel_dfs() and 
	 *                                     pxgstrf_super_bnd_dfs();
	 *                values in [0 : n-1]  when used by pcgstrf_panel_dfs()
	 *                values in [n : 2n-1] when used by pxgstrf_super_bnd_dfs()
	 *	    marker[2m : 3m-1]: used by pcgstrf_column_dfs() in inner-factor 
	 *
	 ...
	 *      the panel diagonal. These are filled in during pcgstrf_panel_dfs(), 
	 ...
	 *                   pcgstrf_panel_dfs() of other panels.
	 *         ENDIF
	 *
	 *     END WHILE;
	 *
...
#endif
    pcgstrf_threadarg_t *thr_arg = &((pcgstrf_threadarg_t *)arg)[pnum];
#else
	pcgstrf_threadarg_t* thr_arg = arg;
	...
	int_t* lbusy; /* "Local busy" array, indicates which descendants
			 ...
			 during pcgstrf_panel_dfs() and dpanel_bmod(). */
	...
	if((*info = pcgstrf_WorkInit(m, panel_size, &iwork, &dwork))) {
		*info += pcgstrf_memory_use(Glu->nzlmax, Glu->nzumax, Glu->nzlumax);
		return 0;
	}
	pxgstrf_SetIWork(m, panel_size, iwork, &segrep, &parent, &xplore,
	                 &repfnz, &panel_lsub, &marker, &lbusy);
	pcgstrf_SetRWork(m, panel_size, dwork, &dense, &tempv);
	...
	Gstat->procstat[pnum].skedtime += t2;	    
		...
		pdiv = Gstat->procstat[pnum].fcops;
				...
				if(*info) {
					if(*info > n) return 0;
					else if(singular == 0 || *info < singular) singular = *info;
#if ( DEBUGlevel>=1 )
    printf("(%d) After pcgstrf_factor_snode(): singular=%d\n", pnum, singular);
#endif
				}

				/* Release the whole relaxed supernode */
				for(jj = jcol; jj < jcol + w; ++jj) pxgstrf_shared->spin_locks[jj] = 0;
#ifdef PREDICT_OPT
...
#if ( DEBUGlevel>=2 )
  if ( jcol==BADPAN )
    printf("(%d) After pcgstrf_panel_dfs(): nseg1 %d, w_lsub_end %d\n",
	   pnum, nseg1, w_lsub_end[0]);
		...
		utime[DFS] += t2;
				...
				for(k = xlsub[jcolm1]; k < itemp; ++k) marker2[lsub[k]] = jcolm1;
#ifdef PREDICT_OPT
		pdiv = Gstat->procstat[pnum].fcops;
#endif
		    ...
		    utime[DFS] += t2;
					...
					if((*info = pcgstrf_pivotL
						...
						if(singular == 0 || *info < singular) {
							singular = *info;
#if ( DEBUGlevel>=1 )
    printf("(%d) After pcgstrf_pivotL(): singular=%d\n", pnum, singular);
#endif
						}
...
/*  if (jj >= LOCOL && jj <= HICOL) {*/
                cprint_lu_col(pnum, "panel:", jcol, jj, w, pivrow, xprune, Glu);
                ccheck_zero_vec(pnum, "after pcgstrf_copy_to_ucol() dense_col[]", n, &dense[k]);
#endif
				} /* for jj ... */
	...
	*info = singular;
	...
	pcgstrf_WorkFree(iwork, dwork, Glu);
