...
int_t qrnzcnt(int_t neqns, int_t adjlen, int_t* xadj, int_t* adjncy, int_t* zfdperm,
	...
	*/
	...
	for(k = 0; k < neqns; ++k) {
		...
		colcnt_h[k] = 0;
		ifdesc = fdesc[k];
		if(ifdesc < fdesc[parent]) { fdesc[parent] = ifdesc; }
	}
	...
	*nlnz = nhnz = 0;
	for(k = 0; k < neqns; ++k) {
		...
		*nlnz += temp;
		parent = etpar[k];
		if(parent != ROOT) { colcnt[parent] += temp; }

		/* for H */
		temp = colcnt_h[k] + weight_h[k];
		colcnt_h[k] = temp;
		nhnz += temp;
		if(parent != ROOT) { colcnt_h[parent] += temp; }
	}
