...
void cgstrs(trans_t trans, SuperMatrix* L, SuperMatrix* U,
...
#endif
#else
				for(j = 0, bptr = fsupc; j < nrhs; j++, bptr += ldb) { cusolve(nsupr, nsupc, &Lval[luptr], &Bmat[bptr]); }
#endif
			}
 ...
 */
void cprint_soln(int_t n, int_t nrhs, complex* soln) {
	int_t i;
