 ...
 */
float pzgstrf_MemInit(int_t n, int_t annz, superlumt_options_t* superlumt_options,
	...
	if(!zexpanders) zexpanders = (ExpHeader*)SUPERLU_MALLOC(NO_MEMTYPE * sizeof(ExpHeader));
	...
	if(refact == NO) {
		...
		} else { pzgstrf_SetupSpace(work, lwork); }
	    ...
	    printf(".. pzgstrf_MemInit(): #retries " IFMT "\n", ++retries);
			...
			if(whichspace == SYSTEM) {
				...
				SUPERLU_FREE(lsub);
				SUPERLU_FREE(usub);
			} else { zuser_free(nzumax * dword + (nzlmax + nzumax) * iword, HEAD); }
 ...
 */
int_t pzgstrf_WorkInit(int_t n, int_t panel_size, int_t** iworkptr, doublecomplex** dworkptr) {
	...
	if(whichspace == SYSTEM) *iworkptr = (int_t*)intCalloc(isize / sizeof(int_t));
	else *iworkptr = (int_t*)zuser_malloc(isize, TAIL);
	if(! *iworkptr) {
		fprintf(stderr, "pzgstrf_WorkInit: malloc fails for local iworkptr[]\n");
		return (isize + n);
	}

	if(whichspace == SYSTEM) *dworkptr = (doublecomplex*)SUPERLU_MALLOC((size_t) dsize);
	else {
 ...
 */
void pzgstrf_SetRWork(int_t n, int_t panel_size, doublecomplex* dworkptr,
...
int_t pzgstrf_MemXpand(
	...
	if(mem_type == USUB) new_mem = pzgstrf_expand(maxlen, mem_type, next, 1, Glu);
	else new_mem = pzgstrf_expand(maxlen, mem_type, next, 0, Glu);
...
void copy_mem_doublecomplex(int_t howmany, void* old, void* new) {
	register int_t i;
...
* pzgstrf_expand(
	...
	if(whichspace == SYSTEM) {
		...
		if(no_expand != 0) {
			...
			if(type == LSUB || type == USUB) { copy_mem_int(len_to_copy, zexpanders[type].mem, new_mem); } else { copy_mem_doublecomplex(len_to_copy, zexpanders[type].mem, new_mem); }
 ...
 */
void pzgstrf_StackCompress(GlobalLU_t* Glu) {
 ...
 */
void zallocateA(int_t n, int_t nnz, doublecomplex** a, int_t** asub, int_t** xa) {
