...
#if ( DEBUGlevel>=2 )
if (jcol == BADPAN)    
    printf("(%d) pcgstrf_panel_dfs[begin] jcol %d, w %d\n", pnum, jcol, w);
#endif
...
#if ( DEBUGlevel>=3 )
  if (jj == BADCOL)		    
    printf("(%d) pcgstrf_panel_dfs(%d) skip busy krow %d, kperm %d\n",
	   pnum, jj, krow, kperm);
#endif
					continue;
				}
...
#ifdef CHK_DFS
if (jj == BADCOL)		
    printf("(%d) pcgstrf_panel_dfs[1] %d, krep %d, fsupc %d, Pr[krow %d] %d, myfnz %d\n",
...
#ifdef DEBUG
if (jj == BADCOL)					
    printf("(%d) pcgstrf_panel_dfs(%d) skip busy kchild %d, chperm %d\n",
...
#ifdef DEBUG
if (jj == BADCOL)				    
    printf("(%d) pcgstrf_panel_dfs[3] %d, krep %d, Pr[kchild %d] %d, chrep %d, fsupc %d, myfnz %d\n",
...
#ifdef CHK_DFS
if (jj == BADCOL)
    printf("(%d) pcgstrf_panel_dfs[4] %d, ispruned[%d] %d, xdfs %d, maxdfs %d\n",
	   pnum, jj, krep, ispruned[krep], xdfs, maxdfs);
#endif
...
#ifdef CHK_DFS
if (jj == BADCOL)			    
    printf("(%d) pcgstrf_panel_dfs(%d) repfnz[%d] %d added to top.list by jj %d\n",
...
#define REPCOL 0    
    krep = REPCOL;
    printf("(%d) pcgstrf_panel_dfs(end) w_lsub_end[jj=%d] %d, repfnz_col[%d] %d\n",
