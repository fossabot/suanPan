...
int_t pcgstrf_column_dfs(
	 ...
	 * Purpose
	 * =======
	 *   pcgstrf_column_dfs() performs a symbolic factorization on column jcol, 
	...
	for(k = 0; k < lsub_end; ++k) {
		krow = col_lsub[k];
		if(perm_r[krow] == EMPTY) {
			/* krow is in L */
			++no_lsub;
			if(marker2[krow] != jcolm1) samesuper = NO; /* row subset test */
			marker2[krow] = jcol;
		}
	}
...
#if ( DEBUGlevel>=2 )
  if (jcol == BADCOL)
    printf("(%d) pcgstrf_column_dfs[1] %d, fstcol %d, lsub_end %d, no_lsub %d, samesuper? %d\n",
...
#if ( DEBUGlevel>=3 )
  if (jcol == BADCOL)
    printf("(%d) pcgstrf_column_dfs[inner]: perm_r[krow=%d] %d\n", pnum, krow, kperm);
#endif
...
#if ( DEBUGlevel>=3 )
  if (jcol == BADCOL)
    printf("(%d) pcgstrf_column_dfs[inner-U]: krep %d, myfnz %d, kperm %d\n",
...
#if ( DEBUGlevel>=3 )
  if (jcol == BADCOL)
    printf("(%d) pcgstrf_column_dfs[inner-dfs] new nseg %d, repfnz[krep=%d] %d\n",
...
#if ( DEBUGlevel>=3 )
  if (jcol == BADCOL)
    printf("(%d) pcgstrf_column_dfs[2]: nextl %d, samesuper? %d\n",
...
#if ( DEBUGlevel>=3 )
  if (jcol == BADCOL)
    printf("(%d) pcgstrf_column_dfs[YES] jcol-1 %d, jcolm1size %d, supno[%d] %d\n",
	   pnum, jcolm1, jcolm1size, jcolm1, nsuper);
#endif
		if(no_lsub != jcolm1size - 1) samesuper = NO; /* Enforce T2 supernode */
		else {
			/* Make sure the number of columns in a supernode does not
			   exceed threshold. */
			fsupc = xsup[nsuper];
			if(jcol - fsupc >= maxsuper) samesuper = NO;
			else {
...
#if ( DEBUGlevel>=3 )
  if (jcol == BADCOL) {
    printf("(%d) pcgstrf_column_dfs[3]: %d in prev s-node %d? %d\n",
