...
void sCreate_CompCol_Matrix(SuperMatrix* A, int_t m, int_t n, int_t nnz, float* nzval,
...
void sCreate_CompRow_Matrix(SuperMatrix* A, int_t m, int_t n, int_t nnz, float* nzval,
...
void sCreate_CompCol_Permuted(SuperMatrix* A, int_t m, int_t n, int_t nnz, float* nzval,
 ...
 */
void sCompRow_to_CompCol(int_t m, int_t n, int_t nnz,
...
int_t sPrint_CCS_to_triplets(SuperMatrix* A) {
	...
	for(j = 0; j < A->ncol; ++j) { for(i = Astore->colptr[j]; i < Astore->colptr[j + 1]; ++i) { printf(IFMT IFMT "%20.16e\n", Astore->rowind[i], j, dp[i]); } }
	return 0;
}
...
void sCreate_Dense_Matrix(SuperMatrix* X, int_t m, int_t n, float* x, int_t ldx,
                          Stype_t stype, Dtype_t dtype, Mtype_t mtype) {
	DNformat* Xstore;
...
void sCopy_Dense_Matrix(int_t M, int_t N, float* X, int_t ldx, float* Y, int_t ldy) {
	/*
	 *
	 *  Purpose
	 *  =======
...
void sCreate_SuperNode_Matrix(SuperMatrix* L, int_t m, int_t n, int_t nnz, float* nzval,
...
void sCreate_SuperNode_Permuted(SuperMatrix* L, int_t m, int_t n, int_t nnz,
                                float* nzval,
 ...
 */
void sprint_lu_col(int_t pnum, char* msg, int_t pcol, int_t jcol, int_t w, int_t pivrow,
	...
	for(i = Glu->xusub[jcol]; i < Glu->xusub_end[jcol]; i++) printf("(" IFMT ")\t" IFMT "\t%8e\n", pnum, Glu->usub[i], Glu->ucol[i]);
	...
	for(i = xlsub[fsupc]; i < xlsub_end[fsupc]; ++i) printf("(" IFMT ")\t" IFMT "\t%.8e\n", pnum, lsub[i], lusup[k++]);
 ...
 */
void scheck_zero_vec(int_t pnum, char* msg, int_t n, float* vec) {
	...
	} else { printf(".. Normal exit scheck_zero_vec() ..\n"); }
}

void sGenXtrue(int_t n, int_t nrhs, float* x, int_t ldx) {
	int_t i, j;
	for(j = 0; j < nrhs; ++j) {
		for(i = 0; i < n; ++i) {
 ...
 */
void sFillRHS(trans_t trans, int_t nrhs, float* x, int_t ldx, SuperMatrix* A, SuperMatrix* B) {
	NCformat* Astore;
	float* Aval;
	DNformat* Bstore;
	float* rhs;
	...
	sp_sgemm(trans_c, A->nrow, nrhs, A->ncol, one, A,
	         x, ldx, zero, rhs, ldc);
}
 ...
 */
void sfill(float* a, int_t alen, float dval) {
	register int_t i;
...
void sPrintPerf(SuperMatrix* L, SuperMatrix* U, superlu_memusage_t* superlu_memusage,
