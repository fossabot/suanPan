...
void dgstrs(trans_t trans, SuperMatrix* L, SuperMatrix* U,
		...
		*/
		...
		for (j = 0, bptr = 0; j < nrhs; j++, bptr += ldb) {
			...
			iptr++;
				...
				for(j = 0, bptr = 0; j < nrhs; j++, bptr += ldb) {
					rhs_work = &Bmat[bptr];
					dlsolve((int_t)nsupr, (int_t)nsupc, &Lval[luptr], &rhs_work[fsupc]);
					dmatvec((int_t)nsupr, (int_t)nrow, (int_t)nsupc, &Lval[luptr + nsupc],
...
#endif
#else
				for(j = 0, bptr = fsupc; j < nrhs; j++, bptr += ldb) { dusolve(nsupr, nsupc, &Lval[luptr], &Bmat[bptr]); }
#endif
			}
