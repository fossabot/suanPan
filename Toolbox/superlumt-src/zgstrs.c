...
void zgstrs(trans_t trans, SuperMatrix* L, SuperMatrix* U,
	 ...
	 *         pzgstrf(). Use compressed row subscripts storage for supernodes,
	 *         i.e., L has types: Stype = SCP, Dtype = _D, Mtype = TRLU.
	 *
	 * U       (input) SuperMatrix*
	 *         The factor U from the factorization Pr*A*Pc=L*U as computed by
	 *         pzgstrf(). Use column-wise storage scheme, i.e., U has types:
...
#endif
#else
				for(j = 0, bptr = fsupc; j < nrhs; j++, bptr += ldb) { zusolve(nsupr, nsupc, &Lval[luptr], &Bmat[bptr]); }
#endif
			}
	...
	zprint_soln(n, nrhs, Bmat);
		...
		if(trans == TRANS) {
			...
			for(k = 0; k < nrhs; ++k) {
				...
				sp_ztrsv("U", "C", "N", L, U, &Bmat[k * ldb], info);

				/* Multiply by conj(inv(L')). */
				sp_ztrsv("L", "C", "U", L, U, &Bmat[k * ldb], info);
			}
		}
 ...
 */
void zprint_soln(int_t n, int_t nrhs, doublecomplex* soln) {
	int_t i;

	for(i = 0; i < n; i++) printf("\t" IFMT ": %.10f\t%.10f\n", i, soln[i].r, soln[i].i);
}
