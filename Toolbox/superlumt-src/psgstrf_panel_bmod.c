...
static int_t check_panel_dfs_list(int_t pnum, char* msg, int_t jcol, int_t nseg, int_t* segrep) {
	register int_t k;

	printf("(" IFMT ") psgstrf_panel_bmod(%s) jcol " IFMT ", nseg " IFMT " in top. order ->\n",
	       pnum, msg, jcol, nseg);
	for(k = nseg - 1; k >= 0; --k) printf("(" IFMT ") segrep-" IFMT, pnum, segrep[k]);
	printf("\n");
	fflush(stdout);
	return 0;
}
