...
void countnz(const int_t n, int_t* xprune, int_t* nnzL, int_t* nnzU, GlobalLU_t* Glu) {
	...
	*nnzL = 0;
	...
	for(i = 0; i <= nsuper; i++) {
		...
		irep = SUPER_REP(i);
		if(SINGLETON(supno[irep])) nnzL0 += xprune[irep] - xlsub_end[irep];
		else nnzL0 += xprune[irep] - xlsub[irep];
	}
...
void compressSUP(const int_t n, GlobalLU_t* Glu) {
	...
	for(j = 0; j < n; ++j) {
		jstrt = xlusup[j];
		xlusup[j] = nextlu;
		for(i = jstrt; i < xlusup_end[j]; ++i, ++nextlu) lusup[nextlu] = lusup[i];
		xlusup_end[j] = nextlu;
	}
	xlusup[n] = nextlu;
 ...
 */
void StatAlloc(const int_t n, const int_t nprocs, const int_t panel_size,
...
flops_t LUFactFlops(Gstat_t* Gstat) { return (Gstat->ops[FACT]); }

flops_t LUSolveFlops(Gstat_t* Gstat) { return (Gstat->ops[SOLVE]); }
...
float SpaSize(int_t n, int_t np, float sum_npw) { return (sum_npw * 8 + np * 8 + n * 4) / 1024.; }
...
void PrintSumm(char* type, int_t nfail, int_t nrun, int_t nerrs) {
	if(nfail > 0)
		printf("%3s driver: " IFMT " out of " IFMT " tests failed to pass the threshold\n",
		       type, nfail, nrun);
	else printf("All tests for %3s driver passed the threshold (" IFMT " tests run)\n", type, nrun);

	if(nerrs > 0) printf(IFMT " error messages recorded\n", nerrs);
}
 ...
 */
void print_flops_by_height(int_t n, panstat_t* panstat,
...
int_t CPprofile(const int_t n, cp_panel_t* cp_panel, pxgstrf_shared_t* pxgstrf_shared) {
	...
	for(i = Gstat->cp_firstkid[n]; i != EMPTY; i = Gstat->cp_nextkid[i]) {
		...
		j = (pxgstrf_shared->pan_status[i].size > 0) ? i : (i + pxgstrf_shared->pan_status[i].size);
