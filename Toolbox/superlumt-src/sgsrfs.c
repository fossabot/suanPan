...
void sgsrfs(trans_t trans, SuperMatrix* A, SuperMatrix* L, SuperMatrix* U,
	...
	*info = 0;
	...
	for(j = 0; j < nrhs; ++j) {
	    ...
	    SCOPY(&nrow, Bptr, &ione, work, &ione);
			...
			s = 0.;
			for(i = 0; i < A->nrow; ++i) {
				if(rwork[i] > safe2) { s = SUPERLU_MAX(s, fabs(work[i]) / rwork[i]); } else if(rwork[i] != 0.0) { s = SUPERLU_MAX(s, (fabs(work[i]) + safe1) / rwork[i]); }
