 ...
 */
float psgstrf_MemInit(int_t n, int_t annz, superlumt_options_t* superlumt_options,
	...
	if(!sexpanders) sexpanders = (ExpHeader*)SUPERLU_MALLOC(NO_MEMTYPE * sizeof(ExpHeader));
 ...
 */
int_t psgstrf_WorkInit(int_t n, int_t panel_size, int_t** iworkptr, float** dworkptr) {
	...
	else *iworkptr = (int_t*)suser_malloc(isize, TAIL);
	if(! *iworkptr) {
		fprintf(stderr, "psgstrf_WorkInit: malloc fails for local iworkptr[]\n");
		return (isize + n);
	}

	if(whichspace == SYSTEM) *dworkptr = (float*)SUPERLU_MALLOC((size_t) dsize);
	else {
 ...
 */
void psgstrf_SetRWork(int_t n, int_t panel_size, float* dworkptr,
...
int_t psgstrf_MemXpand(
	...
	if(mem_type == USUB) new_mem = psgstrf_expand(maxlen, mem_type, next, 1, Glu);
	else new_mem = psgstrf_expand(maxlen, mem_type, next, 0, Glu);
...
void copy_mem_float(int_t howmany, void* old, void* new) {
	register int_t i;
	float* dold = old;
	float* dnew = new;
 ...
 */
void psgstrf_StackCompress(GlobalLU_t* Glu) {
 ...
 */
void sallocateA(int_t n, int_t nnz, float** a, int_t** asub, int_t** xa) {
