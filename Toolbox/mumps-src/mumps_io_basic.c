...
MUMPS_INT mumps_set_file(MUMPS_INT type,MUMPS_INT file_number_arg) {
  ...
  char buf[64]; /* for error message */
	...
	if(file_number_arg > ((mumps_files + type)->mumps_io_nb_file) - 1) {
		...
		if((mumps_files + type)->mumps_io_pfile_pointer_array == NULL) { return mumps_io_error(-13, "Allocation problem in low-level OOC layer\n"); }
...
(mumps_io_pfile_pointer_array+(mumps_files+type)->mumps_io_current_file_number)->file= open(name, O_WRONLY | O_CREAT | O_TRUNC, 0666); */
  if((mumps_io_pfile_pointer_array+(mumps_files+type)->mumps_io_current_file_number)->file==-1){
	...
	if((mumps_io_pfile_pointer_array + (mumps_files + type)->mumps_io_current_file_number)->file == NULL) { return mumps_io_error(-90, "Problem while opening OOC file"); }
#endif
	...
	if((mumps_files + type)->mumps_io_current_file_number > (mumps_files + type)->mumps_io_last_file_opened) { (mumps_files + type)->mumps_io_last_file_opened = (mumps_files + type)->mumps_io_current_file_number; }
...
MUMPS_INT mumps_free_file_pointers(MUMPS_INT* step) {
	...
	/*   } */
	for(j = 0; j < bound; j++) {
		if(mumps_files[j].mumps_io_pfile_pointer_array == NULL) { continue; }
      ...
      if(ierr==-1){
        return mumps_io_sys_error(-90,"Problem while closing OOC file");
			...
			if(ierr == -1) { return mumps_io_error(-90, "Problem while closing OOC file\n"); }
#endif
...
MUMPS_INT mumps_io_alloc_file_struct(MUMPS_INT* nb,MUMPS_INT which) {
	...
	if((mumps_files + which)->mumps_io_pfile_pointer_array == NULL) { return mumps_io_error(-13, "Allocation problem in low-level OOC layer\n"); }
	for(i = 0; i < *nb; i++) { (((mumps_files + which)->mumps_io_pfile_pointer_array) + i)->is_opened = 0; }
	return 0;
}
...
MUMPS_INT mumps_init_file_structure(MUMPS_INT* _myid, long long* total_size_io,MUMPS_INT* size_element,MUMPS_INT* nb_file_type,MUMPS_INT* flag_tab) {
  ...
  mumps_flag_open=0;
	...
	if(mumps_files == NULL) { return mumps_io_error(-13, "Allocation problem in low-level OOC layer\n"); }
...
MUMPS_INT mumps_init_file_name(char* mumps_dir, char* mumps_file,
	...
	if(tmp_dir == NULL) { return mumps_io_error(-13, "Allocation problem in low-level OOC layer\n"); }
	tmp_fname = (char*)malloc(((*mumps_dim_file) + 1) * sizeof(char));
	if(tmp_fname == NULL) { return mumps_io_error(-13, "Allocation problem in low-level OOC layer\n"); }
	for(i = 0; i < *mumps_dim_dir; i++) { tmp_dir[i] = mumps_dir[i]; }
	tmp_dir[i] = 0;
	for(i = 0; i < *mumps_dim_file; i++) { tmp_fname[i] = mumps_file[i]; }
	tmp_fname[i] = 0;
...
# endif
	...
	if(!dir_flag) { free(tmp_dir); }
	if(!file_flag) { free(tmp_fname); }
	return 0;
}
	...
	for(j = 0; j < mumps_io_nb_file_type; j++) {
      ...
      if((mumps_io_pfile_pointer_array+i)->file==-1){
        return mumps_io_sys_error(-90,"Problem while opening OOC file");
			...
			if((mumps_io_pfile_pointer_array + i)->file == NULL) { return mumps_io_error(-90, "Problem while opening OOC file"); }
...
MUMPS_INT mumps_io_read_win32__(void* file, void* loc_addr, size_t size,MUMPS_INT local_offset) {
	...
	if((ret_code != size) || (ferror(*(FILE**)file))) { return mumps_io_error(-90, "Problem with I/O operation\n"); }
	return 0;
}
#endif
...
MUMPS_INT mumps_io_write_win32__(void* file, void* loc_addr, size_t write_size, MUMPS_INT where) {
	...
	if((ret_code != write_size) || (ferror(*(FILE**)file))) { return mumps_io_error(-90, "Problem with I/O operation\n"); }
	return 0;
}
#endif
