...
PORD_INT firstPostorder(elimtree_t* T) {
	...
	if((J = T->root) != -1) while(firstchild[J] != -1) J = firstchild[J];
	return (J);
}
...
PORD_INT firstPostorder2(elimtree_t* T, PORD_INT root) {
	...
	if((J = root) != -1) while(firstchild[J] != -1) J = firstchild[J];
	return (J);
}
...
/*****************************************************************************
******************************************************************************/
PORD_INT firstPreorder(elimtree_t* T) { return (T->root); }
...
PORD_INT nextPreorder(elimtree_t* T, PORD_INT J) {
	...
	if(firstchild[J] != -1) J = firstchild[J];
	else {
		while((silbings[J] == -1) && (parent[J] != -1)) J = parent[J];
		J = silbings[J];
	}
	return (J);
}
...
elimtree_t* setupElimTree(graph_t* G, PORD_INT* perm, PORD_INT* invp) {
	...
	for(front = 0; front < nvtx; front++) {
		...
		if(prevlen - 1 == len) ncolupdate[front] = ncolupdate[front - 1] - vwght[u];
		else {
			h = xnzlsub[front] + 1;
			for(i = 1; i < len; i++) {
				hsub = nzlsub[h++];
...
void initFchSilbRoot(elimtree_t* T) {
	...
	for(J = 0; J < nfronts; J++) silbings[J] = firstchild[J] = -1;
...
elimtree_t* permuteElimTree(elimtree_t* T, PORD_INT* perm) {
	...
	/* ---------------------------------------------------------------------
	   ...
	   --------------------------------------------------------------------- */
	for(u = 0; u < nvtx; u++) PTP->vtx2front[perm[u]] = T->vtx2front[u];
...
elimtree_t* expandElimTree(elimtree_t* T, PORD_INT* vtxmap, PORD_INT nvtxorg) {
	...
	for(u = 0; u < nvtxorg; u++) vtx2front2[u] = vtx2front[vtxmap[u]];
	...
	while(J != -1) {
		...
		while((silbings[J] == -1) && (parent[J] != -1)) {
			...
			if((silbings[child] != -1)
				...
				frontmap[J] = cnfronts++;
			else frontmap[J] = frontmap[child];
		}
		J = silbings[J];
	}
...
elimtree_t* mergeFronts(elimtree_t* T, PORD_INT maxzeros) {
	...
	cnfronts = 0;
	for(K = 0; K < nfronts; K++)
		if(rep[K] == K) frontmap[K] = cnfronts++;
		else {
			for(J = K; rep[J] != J; J = rep[J]);
			rep[K] = J;
		}
	for(K = 0; K < nfronts; K++) if((J = rep[K]) != K) frontmap[K] = frontmap[J];
...
elimtree_t* compressElimTree(elimtree_t* T, PORD_INT* frontmap, PORD_INT cnfronts) {
	...
	/* ------------------------------------
	   ...
	   ------------------------------------ */
	for(u = 0; u < nvtx; u++) T2->vtx2front[u] = frontmap[vtx2front[u]];
	return (T2);
}
...
PORD_INT justifyFronts(elimtree_t* T) {
	...
	for(K = firstPostorder(T); K != -1; K = nextPostorder(T, K)) {
		...
		if((child = firstchild[K]) == -1) minWspace[K] = frontsizeK;
		else {
			count = 0;
...
PORD_INT nFactorIndices(elimtree_t* T) {
	...
	ind = 0;
	for(K = 0; K < nfronts; K++) ind += (ncolfactor[K] + ncolupdate[K]);
	return (ind);
}
...
void subtreeFactorOps(elimtree_t* T, FLOAT* ops) {
	...
	for(K = firstPostorder(T); K != -1; K = nextPostorder(T, K)) {
		...
		for(J = T->firstchild[K]; J != -1; J = T->silbings[J]) ops[K] += ops[J];
	}
}
