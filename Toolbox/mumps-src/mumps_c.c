...
mumps_c(MUMPS_STRUC_C* mumps_par) {
	...
	MUMPS_REAL* rinfo;
	MUMPS_REAL* rinfog;
	MUMPS_INT ooc_tmpdir[255];
	MUMPS_INT ooc_prefix[63];
	MUMPS_INT write_problem[255];
	MUMPS_INT save_dir[255];
	MUMPS_INT save_prefix[255];
	/* Other local variables */
	MUMPS_INT idummy;
	MUMPS_INT* idummyp;
	MUMPS_REAL rdummy;
	MUMPS_REAL* rdummyp;
	MUMPS_COMPLEX cdummy;
	MUMPS_COMPLEX* cdummyp;
	...
	if(mumps_par->job == -1) {
		...
		mumps_par->a_elt = 0;
		mumps_par->perm_in = 0;
		mumps_par->sym_perm = 0;
		mumps_par->uns_perm = 0;
		mumps_par->irn_loc = 0;
		mumps_par->jcn_loc = 0;
		mumps_par->a_loc = 0;
		mumps_par->listvar_schur = 0;
		mumps_par->schur = 0;
		mumps_par->mapping = 0;
		mumps_par->pivnul_list = 0;
		mumps_par->colsca = 0;
		mumps_par->colsca_from_mumps = 0;
		mumps_par->rowsca = 0;
		mumps_par->rowsca_from_mumps = 0;
		mumps_par->rhs_sparse = 0;
		mumps_par->irhs_sparse = 0;
		mumps_par->sol_loc = 0;
		mumps_par->rhs_loc = 0;
		mumps_par->irhs_ptr = 0;
		mumps_par->isol_loc = 0;
		mumps_par->irhs_loc = 0;
		...
		mumps_par->nnz_loc = 0;
		mumps_par->nelt = 0;
		mumps_par->instance_number = 0;
		mumps_par->deficiency = 0;
		mumps_par->lwk_user = 0;
		mumps_par->size_schur = 0;
		mumps_par->lrhs = 0;
		mumps_par->lredrhs = 0;
		mumps_par->nrhs = 0;
		mumps_par->nz_rhs = 0;
		mumps_par->lsol_loc = 0;
		mumps_par->nloc_rhs = 0;
		mumps_par->lrhs_loc = 0;
		mumps_par->schur_mloc = 0;
		mumps_par->schur_nloc = 0;
		mumps_par->schur_lld = 0;
		mumps_par->mblock = 0;
		mumps_par->nblock = 0;
		mumps_par->nprow = 0;
		mumps_par->npcol = 0;
	 ...
	 * error.  */
	if(ooc_tmpdirlen > 255) { ooc_tmpdirlen = 255; }
	if(ooc_prefixlen > 63) { ooc_prefixlen = 63; }
	if(write_problemlen > 255) { write_problemlen = 255; }
	if(save_dirlen > 255) { save_dirlen = 255; }
	if(save_prefixlen > 255) { save_prefixlen = 255; }
	/*
...
#define EXTRACT_POINTERS(component,dummypointer) \
	...
	for(i = 0; i < ooc_tmpdirlen; i++) { ooc_tmpdir[i] = (int)mumps_par->ooc_tmpdir[i]; }
	for(i = 0; i < ooc_prefixlen; i++) { ooc_prefix[i] = (int)mumps_par->ooc_prefix[i]; }
	for(i = 0; i < write_problemlen; i++) { write_problem[i] = (int)mumps_par->write_problem[i]; }
	for(i = 0; i < save_dirlen; i++) { save_dir[i] = (int)mumps_par->save_dir[i]; }
	for(i = 0; i < save_prefixlen; i++) { save_prefix[i] = (int)mumps_par->save_prefix[i]; }
	 ...
	 */
	if(mumps_par->job == -1) { mumps_par->keep[499] = 1; }
	/*
	 ...
	 */
	for(i = 0; i < ooc_tmpdirlen; i++) { mumps_par->ooc_tmpdir[i] = (char)ooc_tmpdir[i]; }
	mumps_par->ooc_tmpdir[ooc_tmpdirlen] = '\0';
	for(i = 0; i < ooc_prefixlen; i++) { mumps_par->ooc_prefix[i] = (char)ooc_prefix[i]; }
