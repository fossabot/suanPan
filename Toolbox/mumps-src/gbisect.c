...
void checkSeparator(gbisect_t* Gbisect) {
	...
	for(u = 0; u < nvtx; u++) {
		...
		case GRAY: /* is it a minimal separator? */
			...
			if(!((a) && (b))) printf("WARNING: not a minimal separator (node %d)\n", u);
			break;
...
void constructSeparator(gbisect_t* Gbisect, options_t* options, timings_t* cpus) {
  ...
  checkDomainDecomposition(dd);
	...
	initialDDSep(dd);
	if(dd->cwght[GRAY] > 0) improveDDSep(dd);
  ...
  checkDDSep(dd);
	...
	while(dd->prev != NULL) {
		...
		for(u = 0; u < dd2->G->nvtx; u++) dd2->color[u] = dd->color[dd2->map[u]];
		freeDomainDecomposition(dd);
		if(dd2->cwght[GRAY] > 0) improveDDSep(dd2);
	...
	for(u = 0; u < nvtx; u++) color[u] = dd->color[map[u]];
	freeDomainDecomposition(dd);
	free(map);
}
...
/*****************************************************************************
******************************************************************************/
PORD_INT smoothBy2Layers(gbisect_t* Gbisect, PORD_INT* bipartvertex, PORD_INT* pnX,
  ...
  printf("Dulmage-Mendelsohn decomp. computed\n"
	...
	if(F(cwght[GRAY]-dmwght[SI]+dmwght[BX], cwght[black]-dmwght[BX],
     ...
     printf("exchange SI with BX\n");
		...
		for(i = 0; i < nX + nY; i++) {
			u = bipartvertex[i];
			if(dmflag[map[u]] == SI) color[u] = white;
			if(dmflag[map[u]] == BX) color[u] = GRAY;
		}
	}
	...
	if((F(cwght[GRAY]-dmwght[SR]+dmwght[BR], cwght[black]-dmwght[BR],
     ...
     printf("exchange SR with BR\n");
		...
		for(i = 0; i < nX + nY; i++) {
			u = bipartvertex[i];
			if(dmflag[map[u]] == SR) color[u] = white;
			if(dmflag[map[u]] == BR) color[u] = GRAY;
		}
	}
	...
	for(i = 0; i < nX + nY; i++) {
		u = bipartvertex[i];
		if(color[u] == GRAY) bipartvertex[nX2++] = u;
	}
	*pnX = nX2;
...
void smoothSeparator(gbisect_t* Gbisect, options_t* options) {
	...
	/* ----------------------------------------------------------
	   ...
	   ---------------------------------------------------------- */
	nX = 0;
	for(u = 0; u < nvtx; u++) if(color[u] == GRAY) bipartvertex[nX++] = u;
