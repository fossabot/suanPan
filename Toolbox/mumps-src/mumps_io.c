...
MUMPS_LOW_LEVEL_INIT_PREFIX(MUMPS_INT* dim, char* str, mumps_ftnlen l1) {
	...
	if(*dim > MUMPS_OOC_PREFIX_MAX_LENGTH) MUMPS_OOC_STORE_PREFIXLEN = MUMPS_OOC_PREFIX_MAX_LENGTH;
	for(i = 0; i < MUMPS_OOC_STORE_PREFIXLEN; i++) { MUMPS_OOC_STORE_PREFIX[i] = str[i]; }
	return;
}
...
MUMPS_LOW_LEVEL_INIT_TMPDIR(MUMPS_INT* dim, char* str, mumps_ftnlen l1) {
	...
	if(*dim > MUMPS_OOC_TMPDIR_MAX_LENGTH) MUMPS_OOC_STORE_TMPDIRLEN = MUMPS_OOC_TMPDIR_MAX_LENGTH;
	for(i = 0; i < MUMPS_OOC_STORE_TMPDIRLEN; i++) { MUMPS_OOC_STORE_TMPDIR[i] = str[i]; }
	return;
}
...
MUMPS_LOW_LEVEL_INIT_OOC_C(MUMPS_INT* _myid, MUMPS_INT* total_size_io, MUMPS_INT* size_element,
	...
	for(i = 0; i < nb_file_type_loc; i++) { flag_tab_loc[i] = (MUMPS_INT)flag_tab[i]; }
#if defined(MUMPS_WIN32)
...
MUMPS_CLEAN_IO_DATA_C(MUMPS_INT* myid,MUMPS_INT* step,MUMPS_INT* ierr) {
	...
	if(!mumps_io_is_init_called) { return; }
	switch(mumps_io_flag_async) {
	case IO_SYNC:
		break;
#if ! defined(MUMPS_WIN32) && ! defined(WITHOUT_PTHREAD)
...
MUMPS_OOC_ALLOC_POINTERS_C(MUMPS_INT* nb_file_type,MUMPS_INT* dim,MUMPS_INT* ierr) {
	...
	for(i = 0; i < nb_file_type_loc; i++) { dim_loc[i] = (MUMPS_INT)dim[i]; }
	*ierr = (MUMPS_INT)mumps_io_alloc_pointers(&nb_file_type_loc, dim_loc);
	for(i = 0; i < nb_file_type_loc; i++) { mumps_io_set_last_file(dim_loc + i, &i); }
	free(dim_loc);
	return;
}
...
void MUMPS_CALL
MUMPS_OOC_END_WRITE_C(MUMPS_INT* ierr) { return; }
 ...
 */
MUMPS_INLINE MUMPS_INT mumps_convert_2fint_to_longlong(MUMPS_INT* short_int1, MUMPS_INT* short_int2,
                                                       long long* long_int) {
