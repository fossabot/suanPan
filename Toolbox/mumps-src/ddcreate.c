	...
	for(i = 0; i < nvtx; i++) {
		...
		if(vtype[u] == 2) {
			...
			for(j = jstart; j < jstop; j++) {
				w = adjncy[j];
				if(vtype[w] == 1) {
					if(v == -1) v = rep[w]; /* u adjacent to domain v = rep[w] */
					else if(v != rep[w]) {
...
void mergeMultisecs(graph_t* G, PORD_INT* vtype, PORD_INT* rep) {
	...
	/* ------------------------------------
	   ...
	   ------------------------------------ */
	for(u = 0; u < nvtx; u++) if(vtype[u] == -2) vtype[u] = 2;
	free(tmp);
	free(queue);
}
...
domdec_t* initialDomainDecomposition(graph_t* G, PORD_INT* map, PORD_INT* vtype, PORD_INT* rep) {
	...
	for(i = 0; i < nedgesdd; i++) adjncydd[i] = map[adjncydd[i]];
	for(u = 0; u < nvtxdd; u++) dd->color[u] = dd->map[u] = -1;
	dd->ndom = ndom;
	dd->domwght = domwght;
...
void computePriorities(domdec_t* dd, PORD_INT* msvtxlist, PORD_INT* key, PORD_INT scoretype) {
	...
	case QMRDV: /* maximal relative decrease of variables in quotient graph */
		for(k = 0; k < nlist; k++) {
			...
			istart = xadj[u];
			istop = xadj[u + 1];
			for(i = istart; i < istop; i++) weight += vwght[adjncy[i]];
	...
	for(k = 0; k < nlist; k++) {
		...
		if(vtype[u] == 2) {
			...
			while(v != -1) {
				istart = xadj[v];
				istop = xadj[v + 1];
				for(i = istart; i < istop; i++) tmp[rep[adjncy[i]]] = flag;
...
domdec_t* coarserDomainDecomposition(domdec_t* dd1, PORD_INT* rep) {
	...
	for(u = 0; u < nvtxdd1; u++)
		if(rep[u] == u) {
			...
			if(vtypedd2[nvtxdd2] == 3) vtypedd2[nvtxdd2] = 1;
			tmp[u] = flag;
	...
	for(i = 0; i < nedgesdd2; i++) adjncydd2[i] = mapdd1[adjncydd2[i]];
	for(u = 0; u < nvtxdd2; u++) dd2->color[u] = dd2->map[u] = -1;
	dd2->ndom = ndom;
	dd2->domwght = domwght;
	...
	/* --------------------------
	   ...
	   -------------------------- */
	for(u = 0; u < nvtxdd1; u++) if((vtypedd1[u] == 3) || (vtypedd1[u] == 4)) vtypedd1[u] = 2;
...
void shrinkDomainDecomposition(domdec_t* dd1, PORD_INT scoretype) {
	...
	nlist = 0;
	for(u = 0; u < nvtxdd1; u++) {
		if(dd1->vtype[u] == 2) msvtxlist[nlist++] = u;
		rep[u] = u;
	}
