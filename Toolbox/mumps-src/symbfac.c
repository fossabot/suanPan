...
css_t* setupCSSFromGraph(graph_t* G, PORD_INT* perm, PORD_INT* invp) {
	...
	for(k = 0; k < neqs; k++) marker[k] = mergelink[k] = -1;
	...
	for(k = 0; k < neqs; k++) {
		...
		if((fast) && (mergelink[mergecol] == -1)) {
			...
			len = 0;
			for(i = beg; i < end; i++) nzlsub[i] = indices[len++];
		}
...
void printFactorMtx(factorMtx_t* L) {
	...
	for(k = 0; k < neqs; k++) {
		...
		istart = xnzl[k];
		istop = xnzl[k + 1];
		for(i = istart; i < istop; i++) printf("  row %5d, entry %e\n", nzlsub[ksub++], nzl[i]);
	}
}
...
void initFactorMtxNEW(factorMtx_t* L, inputMtx_t* PAP) {
	...
	for(K = firstPostorder(PTP); K != -1; K = nextPostorder(PTP, K)) {
		...
		istart = xnzf[K];
		istop = xnzf[K + 1];
		for(i = istart; i < istop; i++) tmp[nzfsub[i]] = len++;
