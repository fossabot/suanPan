...
gbipart_t* setupBipartiteGraph(graph_t* G, PORD_INT* bipartvertex, PORD_INT nX, PORD_INT nY, PORD_INT* vtxmap) {
	...
	for(i = 0; i < nX + nY; i++) {
		...
		jstart = xadj[u];
		jstop = xadj[u + 1];
		for(j = jstart; j < jstop; j++) vtxmap[adjncy[j]] = -1;
	...
	for(i = 0; i < nX; i++) {
		...
		for(j = jstart; j < jstop; j++) {
			y = adjncy[j];
			if(vtxmap[y] >= nX) adjncyGb[ptr++] = vtxmap[y];
		}
	}
	for(i = nX; i < nX + nY; i++) {
		y = bipartvertex[i];
		...
		for(j = jstart; j < jstop; j++) {
			x = adjncy[j];
			if((vtxmap[x] >= 0) && (vtxmap[x] < nX)) adjncyGb[ptr++] = vtxmap[x];
		}
	}
	xadjGb[nX + nY] = ptr;
...
void maximumMatching(gbipart_t* Gbipart, PORD_INT* matching) {
	...
	/* -------------------
	   initialize matching
	   ------------------- */
	for(u = 0; u < nX + nY; u++) matching[u] = FREE;
	...
	/* --------------------------------------------------------------------
	   ...
	   -------------------------------------------------------------------- */
	while(TRUE) {
		for(u = 0; u < nX + nY; u++) level[u] = marker[u] = -1;
...
void maximumFlow(gbipart_t* Gbipart, PORD_INT* flow, PORD_INT* rc) {
	...
	/* -------------------------------------
	   ...
	   ------------------------------------- */
	for(u = 0; u < nX + nY; u++) rc[u] = vwght[u];
	for(i = 0; i < nedges; i++) flow[i] = 0;
	...
	/* -----------------------------------------------------------
	   ...
	   ----------------------------------------------------------- */
	while(TRUE) {
		for(u = 0; u < nX + nY; u++) parent[u] = marker[u] = -1;
		qhead = qtail = 0; /* fill queue with free X nodes */
		for(x = 0; x < nX; x++)
			if(rc[x] > 0) {
				queue[qtail++] = x;
					...
					/*            positive capacity (i.e. (u,v) has neg. capacity) */
				{
					...
					if((v >= nX) && (rc[v] > 0)) /* found it! */
					{
						...
						while(parent[u] != u) /* get minimal residual capa. */
						{
							i = marker[u];
							u = parent[u];
							if(u >= nX) capacity = min(capacity, -flow[i]);
						}
