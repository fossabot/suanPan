...
void constructLevelSep(domdec_t* dd, PORD_INT domain) {
	...
	for(u = 0; u < nvtx; u++) {
		...
		if(vtype[u] == 2) deltaW[u] = xadj[u + 1] - xadj[u];
	}
			...
			if(vtype[u] == -1) {
				...
				for(i = istart; i < istop; i++) {
					...
					if(color[v] == WHITE) {
						dW -= weight;
						dS += weight;
					} /* multisec will move to S */
					else if(deltaW[v] == 1) {
						dB += weight;
						dS -= weight;
					} /* multisec will move to B */
				}
				deltaS[u] = dS;
				deltaB[u] = dB;
				deltaW[u] = dW;
...
void initialDDSep(domdec_t* dd) {
	...
	for(u = 0; u < nvtx; u++) color[u] = WHITE;
	...
	for(u = 0; u < nvtx; u++)
		if((vtype[u] == 1) && (color[u] == WHITE)) {
			...
			if(cwght[BLACK] >= cwght[WHITE]) break;
		}
}

/*****************************************************************************
*****************************************************************************/
void updateB2W(bucket_t* w_bucket, bucket_t* b_bucket, domdec_t* dd, PORD_INT domain,
...
/*****************************************************************************
*****************************************************************************/
void updateW2B(bucket_t* w_bucket, bucket_t* b_bucket, domdec_t* dd, PORD_INT domain,
               PORD_INT* tmp_color, PORD_INT* deltaW, PORD_INT* deltaB, PORD_INT* deltaS) {
	PORD_INT *xadj, *adjncy, *vwght, *vtype;
	PORD_INT weight, u, v, i, istart, istop, j, jstart, jstop;
...
INNER_LOOP_START:
  ...
  printf(" domain %d removed from bucket\n", domain);
	...
	/* -------------------------------------------------------------------
	   ...
	   ------------------------------------------------------------------- */
	if(ftail != -1) vtype[ftail] = -(domain + 1); /* append domain */
