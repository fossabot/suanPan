...
gelim_t* setupElimGraph(graph_t* G) {
	...
	for(i = 0; i < nedges; i++) adjncyGelim[i] = adjncy[i];
	...
	for(u = 0; u < nvtx; u++) {
		...
		case UNWEIGHTED:
			...
			break;
		case WEIGHTED:
			for(i = istart; i < istop; i++) deg += vwght[adjncy[i]];
			break;
		default:
...
PORD_INT crunchElimGraph(gelim_t* Gelim) {
	...
	for(u = 0; u < nvtx; u++) {
		...
		if(i != -1)  /* verify that list is non-empty */
		{
			...
			if(len[u] == 0) printf("error: u %d, len %d\n", u, len[u]);
		}
	}
	...
	while(isrc < Gelim->G->nedges) {
		...
		if(u < 0) /* a new adjacency list starts here */
		{
			...
			xadj[u] = idest++;
			for(i = 1; i < len[u]; i++) adjncy[idest++] = adjncy[isrc++];
		}
	}
			...
			for(j = 0; j < ln; j++) {
				...
				if(vwght[v] > 0) {
					...
					if(medestptr == Gelim->maxedges) {
						...
						for(p = medeststart; p < medestptr; p++) adjncy[G->nedges++] = adjncy[p];
...
/*****************************************************************************
******************************************************************************/
void updateAdjncy(gelim_t* Gelim, PORD_INT* reachset, PORD_INT nreach, PORD_INT* tmp, PORD_INT* pflag) {
...
/*****************************************************************************
******************************************************************************/
void findIndNodes(gelim_t* Gelim, PORD_INT* reachset, PORD_INT nreach, PORD_INT* bin, PORD_INT* next,
  ...
  printf("Checking reachset for indistinguishable variables\n");
	...
	for(i = 0; i < nreach; i++) {
		...
		*/
		...
		for(j = jstart; j < jstop; j += jstep) {
			...
			for(jj = j; jj < jjstop; jj++) chk += adjncy[jj];
			chk = chk % nvtx;
		}
	...
	for(i = 0; i < nreach; i++) {
		...
		if(vwght[u] > 0) /* u is a principal variable */
		{
			...
			while(v != -1) {
				jstart = xadj[v];
				jstop = xadj[v] + len[v];
				for(j = jstart; j < jstop; j++) tmp[adjncy[j]] = *pflag;
	...
	for(i = 0; i < nreach; i++) {
		u = reachset[i];
		if(vwght[u] > 0) parent[u] = -1;
	}
}

/*****************************************************************************
******************************************************************************/
void updateDegree(gelim_t* Gelim, PORD_INT* reachset, PORD_INT nreach, PORD_INT* bin) {
...
elimtree_t* extractElimTree(gelim_t* Gelim) {
	...
	for(u = 0; u < nvtx; u++) sib[u] = fch[u] = -1;
  ...
  for (u = 0; u < nvtx; u++)
	...
	for(u = 0; u < nvtx; u++)
		if(score[u] == -2) {
			v = u;
			while((par[v] != -1) && (score[v] == -2)) v = par[v];
