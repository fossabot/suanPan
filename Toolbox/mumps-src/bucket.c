...
bucket_t* setupBucket(PORD_INT maxbin, PORD_INT maxitem, PORD_INT offset) {
	...
	for(i = 0; i <= maxbin; i++) bucket->bin[i] = -1;
	for(u = 0; u <= maxitem; u++) {
...
void insertBucket(bucket_t* bucket, PORD_INT k, PORD_INT item) {
	...
	if(nextitem != -1) bucket->last[nextitem] = item;
...
void removeBucket(bucket_t* bucket, PORD_INT item) {
	...
	if(nextitem != -1) bucket->last[nextitem] = lastitem;
	if(lastitem != -1) bucket->next[lastitem] = nextitem;
	else {
