...
int Converter::process_amplitude(std::ifstream& input_file, std::ofstream& output_file, const std::string& amplitude_name, const std::string& type) {
	...
	if(is_equal(type, "")) {
		...
		if(getline(input_file, line)) {
			...
			suanpan_warning("amplitude: modulated: the starting time is set to the start of current step in suanPan.\n");
			if(pool[0] != 0.) suanpan_warning("amplitude: modulated: currently constant part of modulated amplitude is not supported by suanPan, it can be generated by using an additional constant amplitude.\n");
		}
		...
		if(getline(input_file, line)) {
			...
			suanpan_warning("amplitude: decay: the starting time is set to the start of current step in suanPan.\n");
			if(pool[0] != 0.) suanpan_warning("amplitude: decay: currently constant part of decay amplitude is not supported by suanPan, it can be generated by using an additional constant amplitude.\n");
		}
	} else if(is_equal(type, "periodic")) {
		suanpan_warning("amplitude: periodic: currently periodic amplitude is not supported by suanPan.\n");
		return SUANPAN_SUCCESS;
	}
...
int Converter::process_boundary(std::ifstream& input_file, std::ofstream& output_file, const std::string& amplitude_name, const std::string& type_name) {
	...
	while(input_file.peek() != '*')
		if(getline(input_file, line)) {
			...
			auto t_set_tag = 0;
			if(pool.size() > 1) {
				if(node_set_pool.find(pool[0]) != node_set_pool.end()) {
			...
			if(4 == pool.size()) {
				...
				int amp_tag;
				if(amplitude_name.empty() || amplitude_pool.find(amplitude_name) == amplitude_pool.end()) amp_tag = 0;
				else amp_tag = amplitude_pool[amplitude_name];

				if(type_name.empty() || if_contain(type_name, "displacement")) commmand_name = "groupdisplacement ";
				else if(if_contain(type_name, "velocity")) commmand_name = "groupvelocity ";
				else if(if_contain(type_name, "acceleration")) commmand_name = "groupacceleration ";

				while(dof_a <= dof_b) output_file << commmand_name << ++load_tag << ' ' << amp_tag << ' ' << value << ' ' << dof_a++ << ' ' << t_set_tag << '\n';
			} else if(3 == pool.size()) {
				if(if_contain(pool[2], ".")) {
					...
					output_file << commmand_name << ++load_tag << ' ' << amp_tag << ' ' << value << ' ' << dof << ' ' << t_set_tag << '\n';
				} else {
