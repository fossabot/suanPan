...
int create_new_integrator(const shared_ptr<DomainBase>& domain, istringstream& command) {
	...
	if(is_equal(integrator_type, "Newmark")) {
		...
		if(!get_input(command, damping_zeta)) {
			suanpan_error("create_new_integrator() needs a valid zeta for Rayleigh damping.\n");
			return SUANPAN_SUCCESS;
		}
...
int create_new_modifier(const shared_ptr<DomainBase>& domain, istringstream& command) {
	...
	if(is_equal(modifier_type, "LumpedSimple")) {
		...
		double a, b, c;
		if(!get_input(command, a)) {
			suanpan_error("create_new_modifier() needs three valid numbers.\n");
			return SUANPAN_SUCCESS;
		}
...
int create_new_orientation(const shared_ptr<DomainBase>& domain, istringstream& command) {
	...
	vec xyz(3);
	for(unsigned I = 0; I < 3; ++I) get_input(command, xyz(I));
...
int create_new_recorder(const shared_ptr<DomainBase>& domain, istringstream& command) {
	...
	if(is_equal(object_type, "Frame")) {
		if(!domain->insert(make_shared<FrameRecorder>(tag, to_list(variable_type.c_str()), inverval))) suanpan_error("create_new_recorder() fails to create a new frame recorder.\n");
		return SUANPAN_SUCCESS;
	}
	if(is_equal(object_type, "Visualisation")) {
		unsigned width = 6;
		if(!command.eof() && !get_input(command, width)) width = 6;
		if(!domain->insert(make_shared<VisualisationRecorder>(tag, to_list(variable_type.c_str()), inverval, width))) suanpan_error("create_new_recorder() fails to create a new visualisation recorder.\n");
		return SUANPAN_SUCCESS;
	}
	...
	if(is_equal(object_type, "Node") && !domain->insert(make_shared<NodeRecorder>(tag, uvec(object_tag), to_list(variable_type.c_str()), inverval, true, use_hdf5))) suanpan_error("create_new_recorder() fails to create a new node recorder.\n");
	...
	else if(is_equal(object_type, "Amplitude") && !domain->insert(make_shared<AmplitudeRecorder>(tag, uvec(object_tag), OutputType::AMP, inverval, true, use_hdf5))) suanpan_error("create_new_recorder() fails to create a new amplitude recorder.\n");
		...
		else if(OutputType::M == to_list(variable_type.c_str())) flag = domain->insert(make_shared<GlobalMassRecorder>(tag, inverval, true, use_hdf5));
		else flag = domain->insert(make_shared<GlobalRecorder>(tag, to_list(variable_type.c_str()), inverval, true, use_hdf5));
...
int create_new_plainrecorder(const shared_ptr<DomainBase>& domain, istringstream& command) {
	...
	if(is_equal(object_type, "Node") && !domain->insert(make_shared<NodeRecorder>(tag, uvec(object_tag), to_list(variable_type.c_str()), inverval, true, false))) suanpan_error("create_new_plainrecorder() fails to create a new node recorder.\n");
	else if(is_equal(object_type, "GroupNode") && !domain->insert(make_shared<GroupNodeRecorder>(tag, uvec(object_tag), to_list(variable_type.c_str()), inverval, true, false))) suanpan_error("create_new_plainrecorder() fails to create a new group node recorder.\n");
	else if(is_equal(object_type, "Sum") && !domain->insert(make_shared<SumRecorder>(tag, uvec(object_tag), to_list(variable_type.c_str()), inverval, true, false))) suanpan_error("create_new_plainrecorder() fails to create a new summation recorder.\n");
	...
	else if(is_equal(object_type, "Amplitude") && !domain->insert(make_shared<AmplitudeRecorder>(tag, uvec(object_tag), OutputType::AMP, inverval, true, false))) suanpan_error("create_new_plainrecorder() fails to create a new amplitude recorder.\n");
	else if(is_equal(object_type, "Global")) {
		bool flag;
		...
		else if(OutputType::M == to_list(variable_type.c_str())) flag = domain->insert(make_shared<GlobalMassRecorder>(tag, inverval, true, false));
		else flag = domain->insert(make_shared<GlobalRecorder>(tag, to_list(variable_type.c_str()), inverval, true, false));
		if(!flag) suanpan_error("create_new_hdf5recorder() fails to create a new global recorder.\n");
	}
...
int create_new_hdf5recorder(const shared_ptr<DomainBase>& domain, istringstream& command) {
	...
	if(is_equal(object_type, "Visualisation")) {
		...
		if(!domain->insert(make_shared<VisualisationRecorder>(tag, to_list(variable_type.c_str()), inverval, width, scale))) suanpan_error("create_new_recorder() fails to create a new visualisation recorder.\n");
		return SUANPAN_SUCCESS;
	}
	...
	if(is_equal(object_type, "Node") && !domain->insert(make_shared<NodeRecorder>(tag, uvec(object_tag), to_list(variable_type.c_str()), inverval, true, true))) suanpan_error("create_new_hdf5recorder() fails to create a new node recorder.\n");
	else if(is_equal(object_type, "GroupNode") && !domain->insert(make_shared<GroupNodeRecorder>(tag, uvec(object_tag), to_list(variable_type.c_str()), inverval, true, true))) suanpan_error("create_new_hdf5recorder() fails to create a new group node recorder.\n");
	else if(is_equal(object_type, "Sum") && !domain->insert(make_shared<SumRecorder>(tag, uvec(object_tag), to_list(variable_type.c_str()), inverval, true, true))) suanpan_error("create_new_hdf5recorder() fails to create a new summation recorder.\n");
	...
	else if(is_equal(object_type, "Amplitude") && !domain->insert(make_shared<AmplitudeRecorder>(tag, uvec(object_tag), OutputType::AMP, inverval, true, true))) suanpan_error("create_new_hdf5recorder() fails to create a new amplitude recorder.\n");
	else if(is_equal(object_type, "Global")) {
		bool flag;
		...
		else if(OutputType::M == to_list(variable_type.c_str())) flag = domain->insert(make_shared<GlobalMassRecorder>(tag, inverval, true, true));
		else flag = domain->insert(make_shared<GlobalRecorder>(tag, to_list(variable_type.c_str()), inverval, true, true));
		if(!flag) suanpan_error("create_new_hdf5recorder() fails to create a new global recorder.\n");
	}
...
int create_new_rigidwall(const shared_ptr<DomainBase>& domain, istringstream& command, const bool finite, const bool penalty) {
	...
	for(unsigned I = 0; I < 3; ++I) if(!get_input(command, origin(I))) return SUANPAN_SUCCESS;
	for(unsigned I = 0; I < 3; ++I) if(!get_input(command, norm(I))) return SUANPAN_SUCCESS;

	if(finite) for(unsigned I = 0; I < 3; ++I) if(!get_input(command, edge(I))) return SUANPAN_SUCCESS;
...
int print_command() {
	...
	const auto format = "    %-30s  %s\n";
	suanpan_info(format, "acceleration", "define acceleration");
	suanpan_info(format, "amplitude", "define ampitude");
	...
	suanpan_info(format, "domain", "create/swith to domains");
