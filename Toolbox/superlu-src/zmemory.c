 ...
 * </pre> 
 */
int zLUMemInit(fact_t fact, void* work, int lwork, int m, int n, int annz,
	...
	if(fact != SamePattern_SameRowPerm) {
		...
		} else { zSetupSpace(work, lwork, Glu); }
	...
	if(info) return (info + zmemory_usage(nzlmax, nzumax, nzlumax, n) + n);
...
int zLUWorkInit(int m, int n, int panel_size, int** iworkptr,
	...
	if(Glu->MemModel == SYSTEM) *iworkptr = (int*)intCalloc(isize / sizeof(int));
	else *iworkptr = (int*)zuser_malloc(isize, TAIL, Glu);
	if(! *iworkptr) {
		fprintf(stderr, "zLUWorkInit: malloc fails for local iworkptr[]\n");
		return (isize + n);
	}

	if(Glu->MemModel == SYSTEM) *dworkptr = (doublecomplex*)SUPERLU_MALLOC(dsize);
	else {
 ...
 */
void zSetRWork(int m, int panel_size, doublecomplex* dworkptr,
...
int zLUMemXpand(int jcol,
	...
	if(mem_type == USUB) new_mem = zexpand(maxlen, mem_type, next, 1, Glu);
	else new_mem = zexpand(maxlen, mem_type, next, 0, Glu);
...
void copy_mem_doublecomplex(int howmany, void* old, void* new) {
	register int i;
...
* zexpand(
	...
	if(Glu->MemModel == SYSTEM) {
		...
		if(Glu->num_expansions != 0) {
			...
			if(type == LSUB || type == USUB) { copy_mem_int(len_to_copy, expanders[type].mem, new_mem); } else { copy_mem_doublecomplex(len_to_copy, expanders[type].mem, new_mem); }
...
doublecomplex* doublecomplexMalloc(int n) {
	...
	if(!buf) { ABORT("SUPERLU_MALLOC failed for buf in doublecomplexMalloc()\n"); }
	return (buf);
}
...
doublecomplex* doublecomplexCalloc(int n) {
	...
	doublecomplex zero = {0.0, 0.0};
	buf = (doublecomplex*)SUPERLU_MALLOC((size_t)n * sizeof(doublecomplex));
	if(!buf) { ABORT("SUPERLU_MALLOC failed for buf in doublecomplexCalloc()\n"); }
