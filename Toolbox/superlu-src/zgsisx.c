...
void zgsisx(superlu_options_t* options, SuperMatrix* A, int* perm_c, int* perm_r,
	...
	*info = 0;
	...
	if(nofact) {
		...
		if(mc64) {
			...
			if(info1 != 0) {
				...
				if(equil) {
					...
					/* scale the matrix */
					for(j = 0; j < n; j++) { for(i = colptr[j]; i < colptr[j + 1]; i++) { zd_mult(&nzval[i], &nzval[i], R[rowind[i]] * C[j]); } }
					*equed = 'B';
				}
		...
		if(!mc64 & equil) {
			/* Only perform equilibration, no row perm */
			t0 = SuperLU_timer_();
	...
	if(nrhs > 0) {
		...
		} else if(colequ) { for(j = 0; j < nrhs; ++j) for(i = 0; i < n; ++i) { zd_mult(&Bmat[i+j*ldb], &Bmat[i+j*ldb], C[i]); } }
		...
		/* Transform the solution matrix X to a solution of the original
		   system. */
		if(notran) { if(colequ) { for(j = 0; j < nrhs; ++j) for(i = 0; i < n; ++i) { zd_mult(&Xmat[i+j*ldx], &Xmat[i+j*ldx], C[i]); } } } else {
			/* transposed system */
			if(rowequ) { for(j = 0; j < nrhs; ++j) for(i = 0; i < A->nrow; ++i) { zd_mult(&Xmat[i+j*ldx], &Xmat[i+j*ldx], R[i]); } }
		}
	} /* end if nrhs > 0 */
