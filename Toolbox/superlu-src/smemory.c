 ...
 * </pre> 
 */
int sLUMemInit(fact_t fact, void* work, int lwork, int m, int n, int annz,
	...
	if(fact != SamePattern_SameRowPerm) {
		...
		} else { sSetupSpace(work, lwork, Glu); }
	...
	if(info) return (info + smemory_usage(nzlmax, nzumax, nzlumax, n) + n);
...
int sLUWorkInit(int m, int n, int panel_size, int** iworkptr,
	...
	else *iworkptr = (int*)suser_malloc(isize, TAIL, Glu);
	if(! *iworkptr) {
		fprintf(stderr, "sLUWorkInit: malloc fails for local iworkptr[]\n");
		return (isize + n);
	}

	if(Glu->MemModel == SYSTEM) *dworkptr = (float*)SUPERLU_MALLOC(dsize);
	else {
 ...
 */
void sSetRWork(int m, int panel_size, float* dworkptr,
...
int sLUMemXpand(int jcol,
	...
	if(mem_type == USUB) new_mem = sexpand(maxlen, mem_type, next, 1, Glu);
	else new_mem = sexpand(maxlen, mem_type, next, 0, Glu);
...
void copy_mem_float(int howmany, void* old, void* new) {
	register int i;
	float* dold = old;
	float* dnew = new;
...
* sexpand(
	...
	if(Glu->MemModel == SYSTEM) {
		...
		if(Glu->num_expansions != 0) {
			...
			if(type == LSUB || type == USUB) { copy_mem_int(len_to_copy, expanders[type].mem, new_mem); } else { copy_mem_float(len_to_copy, expanders[type].mem, new_mem); }
...
float* floatMalloc(int n) {
	...
	if(!buf) { ABORT("SUPERLU_MALLOC failed for buf in floatMalloc()\n"); }
	return (buf);
}
...
float* floatCalloc(int n) {
	...
	float zero = 0.0;
	buf = (float*)SUPERLU_MALLOC((size_t)n * sizeof(float));
	if(!buf) { ABORT("SUPERLU_MALLOC failed for buf in floatCalloc()\n"); }
