...
void cgsisx(superlu_options_t* options, SuperMatrix* A, int* perm_c, int* perm_r,
		...
		if(mc64) {
			...
			if(info1 != 0) {
				...
				if(equil) {
					...
					/* scale the matrix */
					for(j = 0; j < n; j++) { for(i = colptr[j]; i < colptr[j + 1]; i++) { cs_mult(&nzval[i], &nzval[i], R[rowind[i]] * C[j]); } }
					*equed = 'B';
				}
