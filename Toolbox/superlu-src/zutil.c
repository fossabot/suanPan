...
void zCreate_Dense_Matrix(SuperMatrix* X, int m, int n, doublecomplex* x, int ldx,
                          Stype_t stype, Dtype_t dtype, Mtype_t mtype) {
	DNformat* Xstore;
...
void zCopy_Dense_Matrix(int M, int N, doublecomplex* X, int ldx,
...
void zCreate_SuperNode_Matrix(SuperMatrix* L, int m, int n, int nnz,
...
void zPrint_SuperNode_Matrix(char* what, SuperMatrix* A) {
	...
	printf("\nrowind: ");
	for(i = 0; i < Astore->rowind_colptr[n]; ++i) printf("%d  ", Astore->rowind[i]);
	printf("\nrowind_colptr: ");
 ...
 */
void zFillRHS(trans_t trans, int nrhs, doublecomplex* x, int ldx,
