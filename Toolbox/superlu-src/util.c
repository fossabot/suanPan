...
void fixupL(const int n, const int* perm_r, GlobalLU_t* Glu) {
	...
	for(i = 0; i <= nsuper; i++) {
		...
		for(k = fsupc + 1; k < xsup[i + 1]; k++) xlsub[k] = nextl; /* Other columns in supernode i */
...
flops_t LUFactFlops(SuperLUStat_t* stat) { return (stat->ops[FACT]); }

flops_t LUSolveFlops(SuperLUStat_t* stat) { return (stat->ops[SOLVE]); }
...
float SpaSize(int n, int np, float sum_npw) { return (sum_npw * 8 + np * 8 + n * 4) / 1024.; }
