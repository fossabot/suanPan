 ...
 * </pre> 
 */
int dLUMemInit(fact_t fact, void* work, int lwork, int m, int n, int annz,
	...
	if(fact != SamePattern_SameRowPerm) {
		...
		} else { dSetupSpace(work, lwork, Glu); }
	...
	if(info) return (info + dmemory_usage(nzlmax, nzumax, nzlumax, n) + n);
...
int dLUWorkInit(int m, int n, int panel_size, int** iworkptr,
	...
	else *iworkptr = (int*)duser_malloc(isize, TAIL, Glu);
	if(! *iworkptr) {
		fprintf(stderr, "dLUWorkInit: malloc fails for local iworkptr[]\n");
		return (isize + n);
	}

	if(Glu->MemModel == SYSTEM) *dworkptr = (double*)SUPERLU_MALLOC(dsize);
	else {
 ...
 */
void dSetRWork(int m, int panel_size, double* dworkptr,
...
int dLUMemXpand(int jcol,
	...
	if(mem_type == USUB) new_mem = dexpand(maxlen, mem_type, next, 1, Glu);
	else new_mem = dexpand(maxlen, mem_type, next, 0, Glu);
...
void copy_mem_double(int howmany, void* old, void* new) {
	register int i;
	double* dold = old;
	double* dnew = new;
...
* dexpand(
	...
	if(Glu->MemModel == SYSTEM) {
		...
		if(Glu->num_expansions != 0) {
			...
			if(type == LSUB || type == USUB) { copy_mem_int(len_to_copy, expanders[type].mem, new_mem); } else { copy_mem_double(len_to_copy, expanders[type].mem, new_mem); }
...
double* doubleMalloc(int n) {
	...
	if(!buf) { ABORT("SUPERLU_MALLOC failed for buf in doubleMalloc()\n"); }
	return (buf);
}
...
double* doubleCalloc(int n) {
	...
	double zero = 0.0;
	buf = (double*)SUPERLU_MALLOC((size_t)n * sizeof(double));
	if(!buf) { ABORT("SUPERLU_MALLOC failed for buf in doubleCalloc()\n"); }
