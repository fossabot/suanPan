...
Substepping::Substepping(const Substepping& old_obj)
	...
	, trial_mat_obj(nullptr == old_obj.trial_mat_obj ? nullptr : old_obj.trial_mat_obj->get_copy())
...
void Substepping::initialize(const shared_ptr<DomainBase>& D) {
	...
	current_mat_obj->initialize(D);

	trial_mat_obj = current_mat_obj->get_copy();

	PureWrapper(this);

	access::rw(density) = current_mat_obj->get_parameter();
...
void Substepping::print() { current_mat_obj->print(); }
