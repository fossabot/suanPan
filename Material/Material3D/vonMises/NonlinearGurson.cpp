...
void NonlinearGurson::initialize(const shared_ptr<DomainBase>&) {
...
int NonlinearGurson::update_trial_status(const vec& t_strain) {
	...
	const auto trial_p = tensor::mean(trial_stress);                     // trial hydrostatic stress
	...
	while(true) {
		...
		suanpan_debug("NonlinearGurson local iteraton error: %.5E.\n", error);
