...
void new_bilinearperic(unique_ptr<Material>& return_obj, istringstream& command) {
	...
	if(!get_input(command, epsilion)) {
		suanpan_error("new_bilinearperic() requires a valid epsilion.\n");
		return;
	}
	...
	return_obj = make_unique<BilinearPeric>(tag, elastic_modulus, poissons_ratio, yield_stress, hardening, mu, epsilion, density);
}
...
void new_coulumbfriction(unique_ptr<Material>& return_obj, istringstream& command) {
	unsigned tag;
	if(!get_input(command, tag)) {
		suanpan_error("new_coulumbfriction() requires a valid tag.\n");
		return;
	}

	double max_friction, factor;
	if(!get_input(command, max_friction) || !get_input(command, factor)) {
		suanpan_error("new_coulumbfriction() requires a paramater.\n");
		return;
	}
...
void new_lineardamage(unique_ptr<Material>& return_obj, istringstream& command) {
	...
	if(!command.eof() && !get_input(command, value(2))) {
		suanpan_error("new_lineardamage() requires a valid end danage value.\n");
		return;
	}
