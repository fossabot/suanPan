...
Maxwell::Maxwell(const Maxwell& old_obj)
	...
	, beta(old_obj.beta)
	, damper(nullptr == old_obj.damper ? nullptr : suanpan::make_copy(old_obj.damper))
	, spring(nullptr == old_obj.spring ? nullptr : suanpan::make_copy(old_obj.spring)) {}
...
int Maxwell::update_trial_status(const vec& t_strain, const vec& t_strain_rate) {
	...
	if(1 >= proceed || ++delay_counter == proceed) {
		suanpan_error("Maxwell: local iteration cannot converge within %u interations.\n", max_iteration);
		return SUANPAN_FAIL;
	}
