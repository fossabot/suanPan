...
const unsigned NonlinearGurson1D::max_iteration = 20;
...
void NonlinearGurson1D::initialize(const shared_ptr<DomainBase>&) {
...
vector<vec> NonlinearGurson1D::record(const OutputType P) {
	...
	if(P == OutputType::PE) {
		vector<vec> data;
		data.emplace_back(current_strain - current_stress / elastic_modulus);
		return data;
	}
...
void NonlinearGurson1D::print() { suanpan_info("A uniaxial nonlinear hardening model using Gurson's model.\n"); }
