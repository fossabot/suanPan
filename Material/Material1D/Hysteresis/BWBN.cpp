...
void BWBN::initialize(const shared_ptr<DomainBase>&) {
...
int BWBN::update_trial_status(const vec& t_strain) {
	...
	auto& current_z = current_history(0); // z
	auto& current_e = current_history(1); // energy
	...
	while(true) {
		...
		suanpan_debug("BWBN local iteraton error: %.5E.\n", error);
