...
podarray<double> ConcreteExp::compute_compression_backbone(const double n_strain) const {
	...
	while(true) {
		...
		suanpan_debug("ConcreteExp local compression iteraton error: %.5E.\n", fabs(incre));
...
podarray<double> ConcreteExp::compute_tension_backbone(const double n_strain) const {
	...
	while(true) {
		...
		suanpan_debug("ConcreteExp local tension iteraton error: %.5E.\n", fabs(incre));
		if(fabs(incre) < tolerance) break;
...
void ConcreteExp::initialize(const shared_ptr<DomainBase>&) { trial_stiffness = current_stiffness = initial_stiffness = elastic_modulus; }
