...
void ConcreteTable::initialize(const shared_ptr<DomainBase>&) { trial_stiffness = current_stiffness = initial_stiffness = .5 * ((c_table(1, 1) - c_table(0, 1)) / (c_table(1, 0) - c_table(0, 0)) + (t_table(1, 1) - t_table(0, 1)) / (t_table(1, 0) - t_table(0, 0))); }
