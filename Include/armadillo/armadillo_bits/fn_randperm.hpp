...
void internal_randperm_helper(obj_type& x, const uword N, const uword N_keep) {
...
inline
typename enable_if2<is_Mat<obj_type>::value, obj_type>::result randperm(const uword N) {
	arma_extra_debug_sigprint();
...
template<typename obj_type> arma_warn_unused
inline
typename enable_if2<is_Mat<obj_type>::value, obj_type>::result randperm(const uword N, const uword M) {
	arma_extra_debug_sigprint();
...
inline
uvec randperm(const uword N, const uword M) {
	arma_extra_debug_sigprint();
