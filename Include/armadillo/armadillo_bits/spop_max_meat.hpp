...
void spop_max::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_max>& in) {
...
(
	...
	} else if(dim == 1) // find the maximum in each row
	{
		...
		for(uword row = 0; row < p_n_rows; ++row) { if(count[row] < p_n_cols) { value[row] = (std::max)(value[row], eT(0)); } }
...
template<typename T1> inline
typename T1::elem_type spop_max::vector_max
(
	const T1& x,
	const typename arma_not_cx<typename T1::elem_type>::result* junk
) {
...
template<typename T1> inline
typename arma_not_cx<typename T1::elem_type>::result spop_max::max(const SpBase<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename arma_not_cx<typename T1::elem_type>::result spop_max::max_with_index(const SpProxy<T1>& P, uword& index_of_max_val) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename arma_cx_only<typename T1::elem_type>::result spop_max::max(const SpBase<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename arma_cx_only<typename T1::elem_type>::result spop_max::max_with_index(const SpProxy<T1>& P, uword& index_of_max_val) {
	arma_extra_debug_sigprint();
