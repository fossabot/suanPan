...
void op_prod::apply_noalias(Mat<eT>& out, const Mat<eT>& X, const uword dim) {
...
void op_prod::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_prod>& in) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT op_prod::prod(const subview<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename T1::elem_type op_prod::prod(const Base<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
