...
<
	...
	const mtOp<uword, T1, op_any>>::result any(const T1& X) {
	arma_extra_debug_sigprint();
...
<
	...
	const mtOp<uword, T1, op_any>>::result any(const T1& X, const uword dim) {
	arma_extra_debug_sigprint();
...
<
	...
	bool>::result any(const T& val) { return (val != T(0)); }
