...
template<typename eT> arma_warn_unused
arma_inline
bool is_finite(const eT x, const typename arma_scalar_only<eT>::result* junk = nullptr) {
	arma_ignore(junk);
...
inline
bool is_finite(const Base<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
<
	...
	const Glue<T1, T2, glue_affmul>>::result affmul(const T1& A, const T2& B) {
	arma_extra_debug_sigprint();
