...
void glue_hypot::apply(Mat<typename T1::elem_type>& out, const Glue<T1, T2, glue_hypot>& expr) {
	...
	if(bad_alias == false) { glue_hypot::apply_noalias(out, P1, P2); } else {
		Mat<eT> tmp;
...
template<typename T1, typename T2> inline
void glue_hypot::apply_noalias(Mat<typename T1::elem_type>& out, const Proxy<T1>& P1, const Proxy<T2>& P2) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> inline
void glue_hypot::apply(Cube<typename T1::elem_type>& out, const GlueCube<T1, T2, glue_hypot>& expr) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> inline
void glue_hypot::apply_noalias(Cube<typename T1::elem_type>& out, const ProxyCube<T1>& P1, const ProxyCube<T2>& P2) {
	arma_extra_debug_sigprint();
