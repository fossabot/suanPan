...
void op_range::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_range>& in) {
...
void op_range::apply_noalias(Mat<eT>& out, const Mat<eT>& X, const uword dim) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename T1::elem_type op_range::vector_range(const T1& expr) {
	arma_extra_debug_sigprint();
