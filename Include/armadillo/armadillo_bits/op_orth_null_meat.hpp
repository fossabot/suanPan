...
void op_orth::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_orth>& expr) {
...
template<typename T1> inline
void op_null::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_null>& expr) {
	arma_extra_debug_sigprint();
...
bool op_null::apply_direct(Mat<typename T1::elem_type>& out, const Base<typename T1::elem_type, T1>& expr, typename T1::pod_type tol) {
	...
	if(count < X.n_cols) {
		...
		for(uword i = 0; i < out_n_elem; ++i) { if(std::abs(out_mem[i]) < std::numeric_limits<T>::epsilon()) { out_mem[i] = eT(0); } }
	} else { out.set_size(X.n_cols, 0); }
