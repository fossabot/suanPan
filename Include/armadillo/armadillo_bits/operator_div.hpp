...
<
	...
	const mtOp<typename std::complex<typename T1::pod_type>, T1, op_cx_scalar_div_pre>>::result operator/
(
...
<
	(is_arma_type<T1>::value && is_cx<typename T1::elem_type>::no),
	const mtOp<typename std::complex<typename T1::pod_type>, T1, op_cx_scalar_div_post>>::result operator/
(
	const T1& X,
	const std::complex<typename T1::pod_type>& k
) {
...
<
	...
	const eGlue<T1, T2, eglue_div>>::result operator/
(
	const T1& X,
	const T2& Y
) {
...
<
	...
	const mtGlue<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result, T1, T2, glue_mixed_div>>::result operator/
(
	const T1& X,
	const T2& Y
) {
...
template<typename T1, typename T2, typename op_type> inline
typename
enable_if2
<
	(
