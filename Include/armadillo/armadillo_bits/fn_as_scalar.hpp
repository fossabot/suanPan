...
arma_inline
typename T1::elem_type as_scalar(const Glue<T1, T2, glue_times>& X, const typename arma_not_cx<typename T1::elem_type>::result* junk = nullptr) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
arma_inline
typename arma_scalar_only<T>::result as_scalar(const T& x) { return x; }
