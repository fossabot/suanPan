...
void glue_intersect::apply(Mat<typename T1::elem_type>& out, const Glue<T1, T2, glue_intersect>& X) {
...
template<typename T1, typename T2> inline
void glue_intersect::apply(Mat<typename T1::elem_type>& out, uvec& iA, uvec& iB, const Base<typename T1::elem_type, T1>& A_expr, const Base<typename T1::elem_type, T2>& B_expr, const bool calc_indx) {
	arma_extra_debug_sigprint();
