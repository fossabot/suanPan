...
template<typename obj_type> inline running_stat_vec<obj_type>::~running_stat_vec() {
	arma_extra_debug_sigprint_this(this);
}

template<typename obj_type> inline running_stat_vec<obj_type>::running_stat_vec(const bool in_calc_cov)
	: calc_cov(in_calc_cov) {
	arma_extra_debug_sigprint_this(this);
}

template<typename obj_type> inline running_stat_vec<obj_type>::running_stat_vec(const running_stat_vec<obj_type>& in_rsv)
...
template<typename obj_type> inline
running_stat_vec<obj_type>& running_stat_vec<obj_type>::operator=(const running_stat_vec<obj_type>& in_rsv) {
...
template<typename obj_type> template<typename T1> arma_hot
inline
void running_stat_vec<obj_type>::operator()(const Base<typename running_stat_vec<obj_type>::T, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename obj_type> template<typename T1> arma_hot
inline
void running_stat_vec<obj_type>::operator()(const Base<std::complex<typename running_stat_vec<obj_type>::T>, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename obj_type> inline
void running_stat_vec<obj_type>::reset() {
	arma_extra_debug_sigprint();
...
template<typename obj_type> inline
const typename running_stat_vec<obj_type>::return_type1& running_stat_vec<obj_type>::mean() const {
	arma_extra_debug_sigprint();
...
//! variance
template<typename obj_type> inline
const typename running_stat_vec<obj_type>::return_type2& running_stat_vec<obj_type>::var(const uword norm_type) {
	arma_extra_debug_sigprint();
...
//! standard deviation
template<typename obj_type> inline
typename running_stat_vec<obj_type>::return_type2 running_stat_vec<obj_type>::stddev(const uword norm_type) const {
	arma_extra_debug_sigprint();
...
//! covariance
template<typename obj_type> inline
const Mat<typename running_stat_vec<obj_type>::eT>& running_stat_vec<obj_type>::cov(const uword norm_type) {
	arma_extra_debug_sigprint();
...
template<typename obj_type> inline
const typename running_stat_vec<obj_type>::return_type1& running_stat_vec<obj_type>::min() const {
	arma_extra_debug_sigprint();
...
template<typename obj_type> inline
const typename running_stat_vec<obj_type>::return_type1& running_stat_vec<obj_type>::max() const {
	arma_extra_debug_sigprint();
...
template<typename obj_type> inline
typename running_stat_vec<obj_type>::return_type1 running_stat_vec<obj_type>::range() const {
	arma_extra_debug_sigprint();
...
template<typename obj_type> inline
typename running_stat_vec<obj_type>::T running_stat_vec<obj_type>::count() const {
	arma_extra_debug_sigprint();
...
template<typename obj_type> inline
void running_stat_vec_aux::update_stats
(
	...
	if(N > T(0)) {
		...
		for(uword i = 0; i < n_elem; ++i) {
			...
			if(val < min_val_mem[i]) { min_val_mem[i] = val; }

			if(val > max_val_mem[i]) { max_val_mem[i] = val; }
		...
		if(x.calc_cov) { x.r_cov.zeros(sample.n_elem, sample.n_elem); }
