...
void op_min::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_min>& in) {
	...
	if(U.is_alias(out) == false) { op_min::apply_noalias(out, X, dim); } else {
		Mat<eT> tmp;
...
void op_min::apply_noalias(Mat<eT>& out, const Mat<eT>& X, const uword dim, const typename arma_not_cx<eT>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
	...
	if(dim == 0) {
		...
		for(uword col = 0; col < X_n_cols; ++col) { out_mem[col] = op_min::direct_min(X.colptr(col), X_n_rows); }
	} else if(dim == 1) {
...
template<typename eT> inline
void op_min::apply_noalias(Mat<eT>& out, const Mat<eT>& X, const uword dim, const typename arma_cx_only<eT>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
	...
	if(dim == 0) {
		...
		for(uword row = 0; row < X_n_rows; ++row) { out_mem[row] = op_min::direct_min(X, row); }
	}
}

template<typename T1> inline
void op_min::apply(Cube<typename T1::elem_type>& out, const OpCube<T1, op_min>& in) {
	arma_extra_debug_sigprint();
	...
	if(U.is_alias(out) == false) { op_min::apply_noalias(out, U.M, dim); } else {
		Cube<eT> tmp;
...
template<typename eT> inline
void op_min::apply_noalias(Cube<eT>& out, const Cube<eT>& X, const uword dim, const typename arma_not_cx<eT>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
	...
	if(dim == 0) {
		...
		for(uword slice = 0; slice < X_n_slices; ++slice) {
			...
			for(uword col = 0; col < X_n_cols; ++col) { out_mem[col] = op_min::direct_min(X.slice_colptr(slice, col), X_n_rows); }
		}
	} else if(dim == 1) {
		arma_extra_debug_print("op_min::apply(): dim = 1");
...
template<typename eT> inline
void op_min::apply_noalias(Cube<eT>& out, const Cube<eT>& X, const uword dim, const typename arma_cx_only<eT>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
	...
	if(dim == 0) {
		...
		for(uword slice = 0; slice < X_n_slices; ++slice) {
			...
			for(uword row = 0; row < X_n_rows; ++row) { out_mem[row] = op_min::direct_min(tmp, row); }
		}
	} else if(dim == 2) {
		arma_extra_debug_print("op_min::apply(): dim = 2");
...
template<typename eT> inline
eT op_min::direct_min(const eT* const X, const uword n_elem) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT op_min::direct_min(const eT* const X, const uword n_elem, uword& index_of_min_val) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT op_min::direct_min(const Mat<eT>& X, const uword row) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT op_min::min(const subview<eT>& X) {
	arma_extra_debug_sigprint();
	...
	} else { for(uword col = 0; col < X_n_cols; ++col) { min_val = (std::min)(min_val, op_min::direct_min(X.colptr(col), X_n_rows)); } }

	return min_val;
}

template<typename T1> inline
typename arma_not_cx<typename T1::elem_type>::result op_min::min(const Base<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename arma_not_cx<typename T1::elem_type>::result op_min::min(const BaseCube<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename arma_not_cx<typename T1::elem_type>::result op_min::min_with_index(const Proxy<T1>& P, uword& index_of_min_val) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename arma_not_cx<typename T1::elem_type>::result op_min::min_with_index(const ProxyCube<T1>& P, uword& index_of_min_val) {
	arma_extra_debug_sigprint();
...
std::complex<T> op_min::direct_min(const std::complex<T>* const X, const uword n_elem) {
	arma_extra_debug_sigprint();
...
template<typename T> inline
std::complex<T> op_min::direct_min(const std::complex<T>* const X, const uword n_elem, uword& index_of_min_val) {
	arma_extra_debug_sigprint();
...
template<typename T> inline
std::complex<T> op_min::direct_min(const Mat<std::complex<T>>& X, const uword row) {
	arma_extra_debug_sigprint();
...
template<typename T> inline
std::complex<T> op_min::min(const subview<std::complex<T>>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename arma_cx_only<typename T1::elem_type>::result op_min::min(const Base<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename arma_cx_only<typename T1::elem_type>::result op_min::min(const BaseCube<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename arma_cx_only<typename T1::elem_type>::result op_min::min_with_index(const Proxy<T1>& P, uword& index_of_min_val) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename arma_cx_only<typename T1::elem_type>::result op_min::min_with_index(const ProxyCube<T1>& P, uword& index_of_min_val) {
	arma_extra_debug_sigprint();
