...
template<typename eT> inline arma_counter<eT>::~arma_counter() {
...
template<typename eT> inline
void arma_counter<eT>::operator++(int) { operator++(); }
...
template<typename eT> inline running_stat<eT>::~running_stat() {
	arma_extra_debug_sigprint_this(this);
}
...
template<typename eT> inline
void running_stat<eT>::operator()(const typename running_stat<eT>::T sample) {
...
template<typename eT> inline
void running_stat<eT>::operator()(const std::complex<typename running_stat<eT>::T>& sample) {
	arma_extra_debug_sigprint();
...
//! variance
template<typename eT> inline
typename running_stat<eT>::T running_stat<eT>::var(const uword norm_type) const {
	arma_extra_debug_sigprint();
...
//! standard deviation
template<typename eT> inline
typename running_stat<eT>::T running_stat<eT>::stddev(const uword norm_type) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT running_stat<eT>::range() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename get_pod_type<eT>::result running_stat<eT>::count() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void running_stat_aux::update_stats(running_stat<eT>& x, const eT sample, const typename arma_not_cx<eT>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
	...
	if(N > T(0)) {
		if(sample < x.min_val) { x.min_val = sample; }

		if(sample > x.max_val) { x.max_val = sample; }
...
template<typename eT> inline
void running_stat_aux::update_stats(running_stat<eT>& x, const std::complex<eT>& sample, const typename arma_not_cx<eT>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
template<typename eT> inline
void running_stat_aux::update_stats(running_stat<eT>& x, const typename eT::value_type sample, const typename arma_cx_only<eT>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
template<typename eT> inline
void running_stat_aux::update_stats(running_stat<eT>& x, const eT& sample, const typename arma_cx_only<eT>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
