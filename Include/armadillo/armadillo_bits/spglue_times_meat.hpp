          ...
          for(uword i=0; i < B_n_cols; ++i)
            ...
            out.col(i) = A.cols(indices) * B_col;
				...
				while(B_it != B_it_end) {
					...
					for(uword row = 0; row < out_n_rows; ++row) { out_col[row] += A.at(row, B_it_row) * B_it_val; }
...
template<typename T1, typename T2> inline
void spglue_times_mixed::apply(SpMat<typename eT_promoter<T1, T2>::eT>& out, const mtSpGlue<typename eT_promoter<T1, T2>::eT, T1, T2, spglue_times_mixed>& expr) {
	arma_extra_debug_sigprint();
