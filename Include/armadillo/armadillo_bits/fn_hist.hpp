...
<
	...
	const mtOp<uword, T1, op_hist>>::result hist(const T1& A, const uword n_bins = 10) {
...
<
	...
	const mtGlue<uword, T1, T2, glue_hist_default>>::result hist(const T1& X, const T2& Y) {
	arma_extra_debug_sigprint();
...
<
	is_arma_type<T1>::value && is_arma_type<T2>::value && is_cx<typename T1::elem_type>::no && is_same_type<typename T1::elem_type, typename T2::elem_type>::value,
	const mtGlue<uword, T1, T2, glue_hist>>::result hist(const T1& X, const T2& Y, const uword dim) {
	arma_extra_debug_sigprint();
