...
void op_repelem::apply_noalias(Mat<typename obj::elem_type>& out, const obj& X, const uword copies_per_row, const uword copies_per_col) {
	...
	for(uword col = 0; col < X_n_cols; ++col) {
		...
		for(uword row = 0; row < X_n_rows; ++row) {
			...
			for(uword row_copy = 0; row_copy < copies_per_row; ++row_copy) { out_colptr_first[out_row_offset + row_copy] = copy_value; }
...
void op_repelem::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_repelem>& in) {
	arma_extra_debug_sigprint();
