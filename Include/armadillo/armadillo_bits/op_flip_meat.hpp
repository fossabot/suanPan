...
void op_flipud::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_flipud>& in) {
...
void op_flipud::apply_direct(Mat<eT>& out, const Mat<eT>& X) {
	arma_extra_debug_sigprint();
	...
	} else // in-place operation
	{
		...
		if(X_n_cols == 1) {
			...
			for(uword col = 0; col < X_n_cols; ++col) {
				...
				for(uword row = 0; row < N; ++row) { std::swap(out_colmem[X_n_rows_m1 - row], out_colmem[row]); }
			}
		}
	}
}
...
template<typename T1> inline
void op_flipud::apply_proxy_noalias(Mat<typename T1::elem_type>& out, const Proxy<T1>& P) {
	arma_extra_debug_sigprint();
	...
	if(((T1::is_col) || (P_n_cols == 1)) && (Proxy<T1>::use_at == false)) {
		...
		for(uword col = 0; col < P_n_cols; ++col) {
			...
			for(uword row = 0; row < P_n_rows; ++row) { out_colmem[P_n_rows_m1 - row] = P.at(row, col); }
		}
	}
}
...
template<typename T1> inline
void op_fliplr::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_fliplr>& in) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void op_fliplr::apply_direct(Mat<eT>& out, const Mat<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_fliplr::apply_proxy_noalias(Mat<typename T1::elem_type>& out, const Proxy<T1>& P) {
	arma_extra_debug_sigprint();
	...
	if(((T1::is_row) || (P_n_rows == 1)) && (Proxy<T1>::use_at == false)) {
		...
		for(uword col = 0; col < P_n_cols; ++col) {
			...
			for(uword row = 0; row < P_n_rows; ++row) { out_colmem[row] = P.at(row, col); }
		}
	}
}
