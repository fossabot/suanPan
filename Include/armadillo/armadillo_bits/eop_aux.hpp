...
public:
	...
	template<typename eT> arma_inline static typename arma_cx_only<eT>::result log1p(const eT& x) {
		arma_ignore(x);
		return eT(0);
	}
	...
	template<typename eT> arma_inline static typename arma_cx_only<eT>::result expm1(const eT& x) {
		arma_ignore(x);
		return eT(0);
	}
