...
template<typename eT, typename T1, typename T2, typename T3> inline
void glue_join_cols::apply(Mat<eT>& out, const Base<eT, T1>& A_expr, const Base<eT, T2>& B_expr, const Base<eT, T3>& C_expr) {
	arma_extra_debug_sigprint();
	...
	if(A.n_elem > 0) {
		row_end_p1 += A.n_rows;
		out.rows(row_start, row_end_p1 - 1) = A;
	}
	...
	if(B.n_elem > 0) {
		row_end_p1 += B.n_rows;
		out.rows(row_start, row_end_p1 - 1) = B;
	}
	...
	if(C.n_elem > 0) {
		row_end_p1 += C.n_rows;
		out.rows(row_start, row_end_p1 - 1) = C;
	}
}

template<typename eT, typename T1, typename T2, typename T3, typename T4> inline
void glue_join_cols::apply(Mat<eT>& out, const Base<eT, T1>& A_expr, const Base<eT, T2>& B_expr, const Base<eT, T3>& C_expr, const Base<eT, T4>& D_expr) {
	arma_extra_debug_sigprint();
	...
	if(D.n_elem > 0) {
		row_end_p1 += D.n_rows;
		out.rows(row_start, row_end_p1 - 1) = D;
	}
}
...
template<typename eT, typename T1, typename T2, typename T3> inline
void glue_join_rows::apply(Mat<eT>& out, const Base<eT, T1>& A_expr, const Base<eT, T2>& B_expr, const Base<eT, T3>& C_expr) {
	arma_extra_debug_sigprint();
	...
	if(A.n_elem > 0) {
		col_end_p1 += A.n_cols;
		out.cols(col_start, col_end_p1 - 1) = A;
	}
	...
	if(B.n_elem > 0) {
		col_end_p1 += B.n_cols;
		out.cols(col_start, col_end_p1 - 1) = B;
	}
	...
	if(C.n_elem > 0) {
		col_end_p1 += C.n_cols;
		out.cols(col_start, col_end_p1 - 1) = C;
	}
}

template<typename eT, typename T1, typename T2, typename T3, typename T4> inline
void glue_join_rows::apply(Mat<eT>& out, const Base<eT, T1>& A_expr, const Base<eT, T2>& B_expr, const Base<eT, T3>& C_expr, const Base<eT, T4>& D_expr) {
	arma_extra_debug_sigprint();
	...
	if(D.n_elem > 0) {
		col_end_p1 += D.n_cols;
		out.cols(col_start, col_end_p1 - 1) = D;
	}
}
