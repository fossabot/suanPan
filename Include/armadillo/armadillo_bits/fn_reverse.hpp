...
<
	...
	const Op<T1, op_reverse_vec>>::result reverse
(
	const T1& X
) {
...
<
	...
	const Op<T1, op_reverse>>::result reverse
(
	const T1& X
) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
typename enable_if2<is_arma_type<T1>::value, const Op<T1, op_reverse>>::result reverse
(
	const T1& X,
	const uword dim
) {
