...
(
	...
	{
		if((settings.id != 0) && ((opts.symmetric) || (opts.pivot_thresh != double(1)))) { arma_debug_warn("spsolve(): ignoring settings not applicable to LAPACK based solver"); }
	...
	if((status == true) && (rcond > T(0)) && (rcond < auxlib::epsilon_lapack(out))) { arma_debug_warn("solve(): solution computed, but system seems singular to working precision (rcond: ", rcond, ")"); }
