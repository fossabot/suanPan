...
<
	...
	const mtOp<std::complex<typename T1::pod_type>, T1, op_roots>>::result roots(const Base<typename T1::elem_type, T1>& X) {
...
<
	is_supported_blas_type<typename T1::elem_type>::value,
	bool>::result roots(Mat<std::complex<typename T1::pod_type>>& out, const Base<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
