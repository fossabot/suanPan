...
template<> template<typename eT> arma_hot arma_inline eT eop_core<eop_log1p>::process(const eT val, const eT) { return eop_aux::log1p(val); }
...
template<> template<typename eT> arma_hot arma_inline eT eop_core<eop_expm1>::process(const eT val, const eT) { return eop_aux::expm1(val); }
