...
namespace newarp {
	...
	template<typename eT> inline UpperHessenbergEigen<eT>::UpperHessenbergEigen()
		: n(0)
		, computed(false) { arma_extra_debug_sigprint(); }

	template<typename eT> inline UpperHessenbergEigen<eT>::UpperHessenbergEigen(const Mat<eT>& mat_obj)
		: n(mat_obj.n_rows)
		, computed(false) {
	...
	void UpperHessenbergEigen<eT>::compute(const Mat<eT>& mat_obj) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	Col<std::complex<eT>> UpperHessenbergEigen<eT>::eigenvalues() {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	Mat<std::complex<eT>> UpperHessenbergEigen<eT>::eigenvectors() {
		arma_extra_debug_sigprint();
