...
template<typename T1, typename T2> class diagmat_proxy<Glue<T1, T2, glue_times>> {
public:

	typedef typename T1::elem_type elem_type;
	typedef typename get_pod_type<elem_type>::result pod_type;

	inline diagmat_proxy(const Glue<T1, T2, glue_times>& X) {
...
template<typename T1, typename T2> class diagmat_proxy_check<Glue<T1, T2, glue_times>> {
public:

	typedef typename T1::elem_type elem_type;
	typedef typename get_pod_type<elem_type>::result pod_type;

	inline diagmat_proxy_check(const Glue<T1, T2, glue_times>& X, const Mat<elem_type>&) {
		op_diagmat::apply_times(P, X.A, X.B);
