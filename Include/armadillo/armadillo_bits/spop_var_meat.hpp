...
void spop_var::apply(SpMat<typename T1::pod_type>& out, const mtSpOp<typename T1::pod_type, T1, spop_var>& in) {
...
template<typename T1> inline
typename T1::pod_type spop_var::var_vec
(
	const T1& X,
	const uword norm_type
) {
...
template<typename T1, typename eT> inline
typename get_pod_type<eT>::result spop_var::iterator_var
(
	T1& it,
	const T1& end,
	const uword n_zero,
