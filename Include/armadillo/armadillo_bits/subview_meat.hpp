...
void subview<eT>::inplace_op(const Base<eT, T1>& in, const char* identifier) {
	...
	if((is_Mat<typename Proxy<T1>::stored_type>::value) || (use_mp) || (has_overlap)) {
		...
		{
			if((is_same_type<op_type, op_internal_equ>::yes) && (s.aux_row1 == 0) && (s_n_rows == s.m.n_rows)) { arrayops::copy(s.colptr(0), B.memptr(), s.n_elem); } else {
				for(uword ucol = 0; ucol < s_n_cols; ++ucol) {
...
template<typename eT> inline
void subview<eT>::clean(const typename get_pod_type<eT>::result threshold) {
	arma_extra_debug_sigprint();
	...
	for(uword ucol = 0; ucol < s_n_cols; ++ucol) { arrayops::clean(s.colptr(ucol), s_n_rows, threshold); }
}
...
void subview<eT>::fill(const eT val) {
	...
	if(s_n_rows == 1) {
		...
		for(uword ucol = 0; ucol < s_n_cols; ++ucol) { arrayops::inplace_set(s.colptr(ucol), val, s_n_rows); }
	}
}
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::is_zero(const typename get_pod_type<eT>::result tol) const {
	arma_extra_debug_sigprint();

	const uword local_n_rows = n_rows;
	const uword local_n_cols = n_cols;

	for(uword ii = 0; ii < local_n_cols; ++ii) { if(arrayops::is_zero(colptr(ii), local_n_rows, tol) == false) { return false; } }
...
template<typename eT> inline
arma_warn_unused
eT& subview<eT>::row_iterator::operator*() { return M->at(current_row, current_col); }
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::row_iterator::operator==(const row_iterator& rhs) const { return ((current_row == rhs.current_row) && (current_col == rhs.current_col)); }
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::row_iterator::operator==(const const_row_iterator& rhs) const { return ((current_row == rhs.current_row) && (current_col == rhs.current_col)); }
...
template<typename eT> inline
arma_warn_unused
const eT& subview<eT>::const_row_iterator::operator*() const { return M->at(current_row, current_col); }
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::const_row_iterator::operator==(const row_iterator& rhs) const { return ((current_row == rhs.current_row) && (current_col == rhs.current_col)); }
...
template<typename eT> inline
arma_warn_unused
bool subview<eT>::const_row_iterator::operator==(const const_row_iterator& rhs) const { return ((current_row == rhs.current_row) && (current_col == rhs.current_col)); }
...
template<typename eT> arma_inline
const Op<subview_col<eT>, op_strans> subview_col<eT>::as_row() const { return Op<subview_col<eT>, op_strans>(*this); }
...
template<typename eT> arma_inline
const Op<subview_row<eT>, op_strans> subview_row<eT>::as_col() const { return Op<subview_row<eT>, op_strans>(*this); }
