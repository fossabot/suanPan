...
template<typename T1> struct unwrap_spmat {
	...
	template<typename eT2> constexpr bool is_alias(const SpMat<eT2>&) const { return false; }
};
...
template<typename out_eT, typename T1, typename T2, typename spglue_type> struct unwrap_spmat<mtSpGlue<out_eT, T1, T2, spglue_type>> {
	typedef SpMat<out_eT> stored_type;

	inline unwrap_spmat(const mtSpGlue<out_eT, T1, T2, spglue_type>& A)
		: M(A) {
		arma_extra_debug_sigprint();
	}
