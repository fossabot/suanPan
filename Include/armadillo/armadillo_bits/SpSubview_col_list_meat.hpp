...
template<typename eT, typename T1> inline SpSubview_col_list<eT, T1>::~SpSubview_col_list() {
	arma_extra_debug_sigprint();
}

template<typename eT, typename T1> arma_inline SpSubview_col_list<eT, T1>::SpSubview_col_list
(
	const SpMat<eT>& in_m,
...
template<typename eT, typename T1> template<typename functor> inline
void SpSubview_col_list<eT, T1>::for_each(functor F) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> template<typename functor> inline
void SpSubview_col_list<eT, T1>::for_each(functor F) const {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> template<typename functor> inline
void SpSubview_col_list<eT, T1>::transform(functor F) {
	arma_extra_debug_sigprint();
...
void SpSubview_col_list<eT, T1>::replace(const eT old_val, const eT new_val) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
void SpSubview_col_list<eT, T1>::clean(const typename get_pod_type<eT>::result threshold) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
void SpSubview_col_list<eT, T1>::fill(const eT val) {
	arma_extra_debug_sigprint();

	Mat<eT> tmp(m.n_rows, U_ci.M.n_elem);
	tmp.fill(val);
...
template<typename eT, typename T1> inline
void SpSubview_col_list<eT, T1>::zeros() {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
void SpSubview_col_list<eT, T1>::ones() {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
void SpSubview_col_list<eT, T1>::operator+=(const eT val) {
	arma_extra_debug_sigprint();

	const SpMat<eT> tmp1(*this);

	Mat<eT> tmp2(tmp1.n_rows, tmp1.n_cols);
	tmp2.fill(val);
...
void SpSubview_col_list<eT, T1>::operator=(const Base<eT, expr>& x) {
	arma_extra_debug_sigprint();
	...
	for(uword i = 0; i < Y.n_cols; ++i) { access::rw(Y.col_ptrs[i + 1]) += Y.col_ptrs[i]; }
...
template<typename eT, typename T1> inline
void SpSubview_col_list<eT, T1>::operator=(const SpSubview_col_list<eT, T1>& x) {
	arma_extra_debug_sigprint();
...
void SpSubview_col_list<eT, T1>::operator=(const SpSubview_col_list<eT, T2>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> template<typename expr> inline
void SpSubview_col_list<eT, T1>::operator=(const SpBase<eT, expr>& x) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
void SpSubview_col_list<eT, T1>::extract(SpMat<eT>& out, const SpSubview_col_list<eT, T1>& in) {
	arma_extra_debug_sigprint();
	...
	// fix the column pointers
	for(uword i = 0; i < out.n_cols; ++i) { access::rw(out.col_ptrs[i + 1]) += out.col_ptrs[i]; }
}

template<typename eT, typename T1> inline
void SpSubview_col_list<eT, T1>::plus_inplace(SpMat<eT>& out, const SpSubview_col_list& in) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
void SpSubview_col_list<eT, T1>::minus_inplace(SpMat<eT>& out, const SpSubview_col_list& in) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
void SpSubview_col_list<eT, T1>::schur_inplace(SpMat<eT>& out, const SpSubview_col_list& in) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1> inline
void SpSubview_col_list<eT, T1>::div_inplace(SpMat<eT>& out, const SpSubview_col_list& in) {
	arma_extra_debug_sigprint();
