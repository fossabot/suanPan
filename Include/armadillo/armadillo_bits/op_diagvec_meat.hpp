...
template<typename T1> inline
void op_diagvec::apply_proxy(Mat<typename T1::elem_type>& out, const Proxy<T1>& P) {
	arma_extra_debug_sigprint();
	...
	if(i < len) { out_mem[i] = P.at(i, i); }
}

template<typename T1, typename T2> inline
void op_diagvec::apply(Mat<typename T1::elem_type>& actual_out, const Op<Glue<T1, T2, glue_times>, op_diagvec>& X, const typename arma_not_cx<typename T1::elem_type>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
template<typename T1, typename T2> inline
void op_diagvec::apply(Mat<typename T1::elem_type>& actual_out, const Op<Glue<T1, T2, glue_times>, op_diagvec>& X, const typename arma_cx_only<typename T1::elem_type>::result* junk) {
	arma_extra_debug_sigprint();
	arma_ignore(junk);
...
template<typename T1> inline
void op_diagvec2::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_diagvec2>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void op_diagvec2::apply_proxy(Mat<typename T1::elem_type>& out, const Proxy<T1>& P, const uword row_offset, const uword col_offset) {
	arma_extra_debug_sigprint();
