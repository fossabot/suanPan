...
inline
typename enable_if2<is_arma_type<T1>::value && resolves_to_vector<T1>::yes, typename T1::elem_type>::result max(const T1& X) {
...
template<typename T1> arma_warn_unused
arma_inline
typename enable_if2<is_arma_type<T1>::value && resolves_to_vector<T1>::no, const Op<T1, op_max>>::result max(const T1& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
arma_inline
typename enable_if2<is_arma_type<T1>::value, const Op<T1, op_max>>::result max(const T1& X, const uword dim) {
	arma_extra_debug_sigprint();
...
arma_inline
typename arma_scalar_only<T>::result max(const T& x) { return x; }
...
<
	...
	const Glue<T1, T2, glue_max>>::result max
(
	const T1& X,
	const T2& Y
) {
...
<
	...
	const SpOp<T1, spop_max>>::result max(const T1& X) {
	arma_extra_debug_sigprint();
...
<
	...
	const SpOp<T1, spop_max>>::result max(const T1& X, const uword dim) {
	arma_extra_debug_sigprint();
...
<
	...
	const SpGlue<T1, T2, spglue_max>>::result max(const T1& x, const T2& y) {
	arma_extra_debug_sigprint();
