...
namespace newarp {
	...
	template<typename eT> inline UpperHessenbergQR<eT>::UpperHessenbergQR()
		: n(0)
		, computed(false) { arma_extra_debug_sigprint(); }

	template<typename eT> inline UpperHessenbergQR<eT>::UpperHessenbergQR(const Mat<eT>& mat_obj)
		: n(mat_obj.n_rows)
		, mat_T(n, n)
		, rot_cos(n - 1)
		, rot_sin(n - 1)
	...
	void UpperHessenbergQR<eT>::apply_YQ(Mat<eT>& Y) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline TridiagQR<eT>::TridiagQR()
		: UpperHessenbergQR<eT>() { arma_extra_debug_sigprint(); }

	template<typename eT> inline TridiagQR<eT>::TridiagQR(const Mat<eT>& mat_obj)
	...
	template<typename eT> inline
	void TridiagQR<eT>::compute(const Mat<eT>& mat_obj) {
		arma_extra_debug_sigprint();
