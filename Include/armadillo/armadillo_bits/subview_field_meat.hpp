...
template<typename oT> inline subview_field<oT>::~subview_field() {
	arma_extra_debug_sigprint();
}

template<typename oT> arma_inline subview_field<oT>::subview_field
(
	const field<oT>& in_f,
	const uword in_row1,
	const uword in_col1,
...
void subview_field<oT>::operator=(const field<oT>& x) {
	...
	if(t.n_slices == 1) {
		for(uword col = 0; col < t.n_cols; ++col)
			for(uword row = 0; row < t.n_rows; ++row) { t.at(row, col) = x.at(row, col); }
	} else {
		for(uword slice = 0; slice < t.n_slices; ++slice)
			for(uword col = 0; col < t.n_cols; ++col)
				for(uword row = 0; row < t.n_rows; ++row) { t.at(row, col, slice) = x.at(row, col, slice); }
	}
}
...
oT& subview_field<oT>::operator[](const uword i) {
...
template<typename oT> arma_inline
const oT& subview_field<oT>::operator[](const uword i) const {
	const uword n_elem_slice = n_rows * n_cols;
...
template<typename oT> arma_inline
oT& subview_field<oT>::operator()(const uword in_row, const uword in_col) { return operator()(in_row, in_col, 0); }

template<typename oT> arma_inline
const oT& subview_field<oT>::operator()(const uword in_row, const uword in_col) const { return operator()(in_row, in_col, 0); }

template<typename oT> arma_inline
oT& subview_field<oT>::operator()(const uword in_row, const uword in_col, const uword in_slice) {
...
template<typename oT> arma_inline
const oT& subview_field<oT>::operator()(const uword in_row, const uword in_col, const uword in_slice) const {
	arma_debug_check(((in_row >= n_rows) || (in_col >= n_cols) || (in_slice >= n_slices)), "subview_field::operator(): index out of bounds");
...
template<typename oT> arma_inline
oT& subview_field<oT>::at(const uword in_row, const uword in_col) { return at(in_row, in_col, 0); }

template<typename oT> arma_inline
const oT& subview_field<oT>::at(const uword in_row, const uword in_col) const { return at(in_row, in_col, 0); }

template<typename oT> arma_inline
oT& subview_field<oT>::at(const uword in_row, const uword in_col, const uword in_slice) {
	const uword index = (in_slice + aux_slice1) * (f.n_rows * f.n_cols) + (in_col + aux_col1) * f.n_rows + aux_row1 + in_row;
...
template<typename oT> arma_inline
const oT& subview_field<oT>::at(const uword in_row, const uword in_col, const uword in_slice) const {
	const uword index = (in_slice + aux_slice1) * (f.n_rows * f.n_cols) + (in_col + aux_col1) * f.n_rows + aux_row1 + in_row;
...
template<typename oT> inline
void subview_field<oT>::print(const std::string extra_text) const {
	arma_extra_debug_sigprint();
...
template<typename oT> inline
void subview_field<oT>::print(std::ostream& user_stream, const std::string extra_text) const {
	arma_extra_debug_sigprint();
...
template<typename oT> template<typename functor> inline
void subview_field<oT>::for_each(functor F) {
	arma_extra_debug_sigprint();
	...
	if(t.n_slices == 1) {
		for(uword col = 0; col < t.n_cols; ++col)
			for(uword row = 0; row < t.n_rows; ++row) { F(t.at(row, col)); }
	} else {
		for(uword slice = 0; slice < t.n_slices; ++slice)
			for(uword col = 0; col < t.n_cols; ++col)
				for(uword row = 0; row < t.n_rows; ++row) { F(t.at(row, col, slice)); }
	}
}

template<typename oT> template<typename functor> inline
void subview_field<oT>::for_each(functor F) const {
	arma_extra_debug_sigprint();
...
template<typename oT> inline
void subview_field<oT>::fill(const oT& x) {
	arma_extra_debug_sigprint();
	...
	if(t.n_slices == 1) {
		for(uword col = 0; col < t.n_cols; ++col)
			for(uword row = 0; row < t.n_rows; ++row) { t.at(row, col) = x; }
	} else {
		for(uword slice = 0; slice < t.n_slices; ++slice)
			for(uword col = 0; col < t.n_cols; ++col)
				for(uword row = 0; row < t.n_rows; ++row) { t.at(row, col, slice) = x; }
	}
}

//! X = Y.subfield(...)
template<typename oT> inline
void subview_field<oT>::extract(field<oT>& actual_out, const subview_field<oT>& in) {
	arma_extra_debug_sigprint();
	...
	if(n_slices == 1) {
		for(uword col = 0; col < n_cols; ++col)
			for(uword row = 0; row < n_rows; ++row) { out.at(row, col) = in.at(row, col); }
	} else {
		for(uword slice = 0; slice < n_slices; ++slice)
			for(uword col = 0; col < n_cols; ++col)
				for(uword row = 0; row < n_rows; ++row) { out.at(row, col, slice) = in.at(row, col, slice); }
	}
