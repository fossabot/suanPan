...
template<typename eT> inline Mat<eT>::Mat(const SpSubview<eT>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_alloc(0)
...
template<typename eT> inline
Mat<eT>& Mat<eT>::operator=(const SpSubview<eT>& X) {
	arma_extra_debug_sigprint();
...
//! remove specified rows
template<typename eT> template<typename T1> inline
void Mat<eT>::shed_rows(const Base<uword, T1>& indices) {
	arma_extra_debug_sigprint();
	...
	if(arma_config::debug) { for(uword i = 0; i < N; ++i) { arma_debug_check((rows_to_shed_mem[i] >= n_rows), "Mat::shed_rows(): indices out of bounds"); } }
...
//! remove specified columns
template<typename eT> template<typename T1> inline
void Mat<eT>::shed_cols(const Base<uword, T1>& indices) {
	arma_extra_debug_sigprint();
	...
	if(arma_config::debug) { for(uword i = 0; i < N; ++i) { arma_debug_check((cols_to_shed_mem[i] >= n_cols), "Mat::shed_cols(): indices out of bounds"); } }
...
template<typename eT> template<typename T1, typename op_type> inline Mat<eT>::Mat(const CubeToMatOp<T1, op_type>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_alloc(0)
...
template<typename eT> template<typename T1, typename op_type> inline
Mat<eT>& Mat<eT>::operator=(const CubeToMatOp<T1, op_type>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1, typename op_type> inline Mat<eT>::Mat(const SpToDOp<T1, op_type>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_alloc(0)
...
template<typename eT> template<typename T1, typename op_type> inline
Mat<eT>& Mat<eT>::operator=(const SpToDOp<T1, op_type>& X) {
	arma_extra_debug_sigprint();
...
bool Mat<eT>::is_sorted(const char* direction, const uword dim) const {
	...
	if(sig1 == 'a') {
		...
		return (*this).is_sorted_helper(comparator, dim);
	} else if((sig1 == 's') && (sig2 == 'a')) {
		// case: strict ascend

		arma_geq_comparator<eT> comparator;

		return (*this).is_sorted_helper(comparator, dim);
	} else if((sig1 == 's') && (sig2 == 'd')) {
		// case: strict descend
...
template<typename eT> template<typename comparator> inline
arma_warn_unused
bool Mat<eT>::is_sorted_helper(const comparator& comp, const uword dim) const {
	arma_extra_debug_sigprint();
...
void Mat<eT>::reshape(const uword in_rows, const uword in_cols, const uword dim) {
	...
	if(dim == 0) { *this = arma::reshape(*this, in_rows, in_cols); } else if(dim == 1) { *this = arma::reshape(strans(*this), in_rows, in_cols); }
}
...
template<typename eT> inline
const Mat<eT>& Mat<eT>::clean(const typename get_pod_type<eT>::result threshold) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Mat<eT>::save(const csv_name& spec, const file_type type, const bool print_status) const {
	arma_extra_debug_sigprint();
	...
	if((print_status == true) && (save_okay == false)) { arma_debug_warn("Mat::save(): couldn't write to ", spec.filename); }
...
template<typename eT> inline
arma_cold
bool Mat<eT>::load(const csv_name& spec, const file_type type, const bool print_status) {
	arma_extra_debug_sigprint();
	...
	if(print_status) {
		if(load_okay == false) { if(err_msg.length() > 0) { arma_debug_warn("Mat::load(): ", err_msg, spec.filename); } else { arma_debug_warn("Mat::load(): couldn't read ", spec.filename); } } else {
			const uword load_n_cols = (do_trans) ? (*this).n_rows : (*this).n_cols;

			if(with_header && (spec.header_rw.n_elem != load_n_cols)) { arma_debug_warn("Mat::load(): size mistmach between header and matrix"); }
		}
	}
...
template<typename eT> inline
arma_cold
bool Mat<eT>::quiet_save(const csv_name& spec, const file_type type) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool Mat<eT>::quiet_load(const csv_name& spec, const file_type type) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline Mat<eT>::row_iterator::row_iterator(Mat<eT>& in_M, const uword in_row, const uword in_col)
	: M(&in_M)
	, current_row(in_row)
	, current_col(in_col) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_warn_unused
eT& Mat<eT>::row_iterator::operator*() { return M->at(current_row, current_col); }
...
template<typename eT> inline
typename Mat<eT>::row_iterator& Mat<eT>::row_iterator::operator--() {
	if(current_col > 0) { current_col--; } else {
		if(current_row > 0) {
...
template<typename eT> inline
arma_warn_unused
bool Mat<eT>::row_iterator::operator!=(const typename Mat<eT>::row_iterator& X) const { return ((current_row != X.current_row) || (current_col != X.current_col)); }
...
template<typename eT> inline
arma_warn_unused
bool Mat<eT>::row_iterator::operator!=(const typename Mat<eT>::const_row_iterator& X) const { return ((current_row != X.current_row) || (current_col != X.current_col)); }
...
template<typename eT> inline Mat<eT>::const_row_iterator::const_row_iterator(const Mat<eT>& in_M, const uword in_row, const uword in_col)
	: M(&in_M)
	, current_row(in_row)
	, current_col(in_col) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_warn_unused
const eT& Mat<eT>::const_row_iterator::operator*() const { return M->at(current_row, current_col); }
...
template<typename eT> inline
arma_warn_unused
bool Mat<eT>::const_row_iterator::operator!=(const typename Mat<eT>::row_iterator& X) const { return ((current_row != X.current_row) || (current_col != X.current_col)); }
...
template<typename eT> inline
arma_warn_unused
bool Mat<eT>::const_row_iterator::operator!=(const typename Mat<eT>::const_row_iterator& X) const { return ((current_row != X.current_row) || (current_col != X.current_col)); }
