...
template<typename eT> inline xvec_htrans<eT>::xvec_htrans(const eT* const in_mem, const uword in_n_rows, const uword in_n_cols)
	: mem(in_mem)
...
void xvec_htrans<eT>::extract(Mat<eT>& out) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
eT xvec_htrans<eT>::operator[](const uword ii) const { return access::alt_conj(mem[ii]); }

template<typename eT> inline
eT xvec_htrans<eT>::at_alt(const uword ii) const { return access::alt_conj(mem[ii]); }

template<typename eT> inline
eT xvec_htrans<eT>::at(const uword in_row, const uword in_col) const {
