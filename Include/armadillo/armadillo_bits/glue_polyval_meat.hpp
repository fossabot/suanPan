...
void glue_polyval::apply_noalias(Mat<eT>& out, const Mat<eT>& P, const Mat<eT>& X) {
...
void glue_polyval::apply(Mat<typename T1::elem_type>& out, const Glue<T1, T2, glue_polyval>& expr) {
	arma_extra_debug_sigprint();
