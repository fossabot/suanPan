...
typename enable_if2<is_supported_blas_type<typename T1::elem_type>::value, bool>::result eig_sym
(
...
inline
typename enable_if2<is_supported_blas_type<typename T1::elem_type>::value, Col<typename T1::pod_type>>::result eig_sym
(
	const Base<typename T1::elem_type, T1>& X
) {
	arma_extra_debug_sigprint();
