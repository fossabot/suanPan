...
namespace newarp {
	...
	template<typename eT, int SelectionRule> struct SortingTarget {
		...
		typename get_pod_type<eT>::result get(const eT& val) {
			arma_ignore(val);
	...
	template<typename eT> struct SortingTarget<eT, EigsSelect::LARGEST_MAGN> {
		...
		typename get_pod_type<eT>::result get(const eT& val) { return -std::abs(val); }
	};
	...
	template<typename PairType> struct PairComparator {
		...
		bool operator()(const PairType& v1, const PairType& v2) { return v1.first < v2.first; }
	};
	...
	public:
		...
		inline SortEigenvalue(const eT* start, const uword size)
			: pair_sort(size) {
