...
namespace newarp {
	...
	template<typename eT> inline TridiagEigen<eT>::TridiagEigen()
		: n(0)
		, computed(false) { arma_extra_debug_sigprint(); }

	template<typename eT> inline TridiagEigen<eT>::TridiagEigen(const Mat<eT>& mat_obj)
		: n(mat_obj.n_rows)
		, computed(false) {
	...
	void TridiagEigen<eT>::compute(const Mat<eT>& mat_obj) {
		arma_extra_debug_sigprint();
		...
		if(info > 0) {
			arma_stop_runtime_error("lapack::stedc(): failed to compute all eigenvalues");
			return;
		}
	...
	template<typename eT> inline
	Col<eT> TridiagEigen<eT>::eigenvalues() {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	Mat<eT> TridiagEigen<eT>::eigenvectors() {
		arma_extra_debug_sigprint();
