...
namespace csv_opts {
	...
	struct opts_no_header : public opts {
		inline opts_no_header()
			: opts(flag_no_header) {}
	};

	struct opts_with_header : public opts {
		inline opts_with_header()
			: opts(flag_with_header) {}
	};
...
struct csv_name {
	...
	inline csv_name(const std::string& in_filename, const csv_opts::opts& in_opts = csv_opts::no_header)
		: filename(in_filename)
		, opts(in_opts)
		, header_ro(header_junk)
		, header_rw(header_junk) {}

	inline csv_name(const std::string& in_filename, field<std::string>& in_header)
		: filename(in_filename)
		, opts(csv_opts::with_header)
		, header_ro(in_header)
		, header_rw(in_header) {}

	inline csv_name(const std::string& in_filename, const field<std::string>& in_header)
		: filename(in_filename)
		, opts(csv_opts::with_header)
		, header_ro(in_header)
		, header_rw(header_junk) {}

	inline csv_name(const std::string& in_filename, field<std::string>& in_header, const csv_opts::opts& in_opts)
		: filename(in_filename)
		, opts(csv_opts::with_header + in_opts)
		, header_ro(in_header)
		, header_rw(in_header) {}

	inline csv_name(const std::string& in_filename, const field<std::string>& in_header, const csv_opts::opts& in_opts)
		: filename(in_filename)
		, opts(csv_opts::with_header + in_opts)
		, header_ro(in_header)
		, header_rw(header_junk) {}
