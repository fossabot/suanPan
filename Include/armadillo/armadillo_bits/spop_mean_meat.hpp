...
template<typename T1, typename spop_type> inline
typename T1::elem_type spop_mean::mean_all(const SpOp<T1, spop_type>& expr) {
	arma_extra_debug_sigprint();
	...
	if(is_vectorise) { return spop_mean::mean_all(expr.m); }
