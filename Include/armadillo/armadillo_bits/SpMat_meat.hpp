...
template<typename eT> template<typename T1, typename op_type> inline SpMat<eT>::SpMat(const SpToDOp<T1, op_type>& expr)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
SpMat<eT>& SpMat<eT>::operator=(const Op<T1, op_diagmat>& expr) {
	...
	for(uword i = 1; i < n_cols + 1; ++i) { access::rw(col_ptrs[i]) += col_ptrs[i - 1]; }
...
template<typename eT> template<typename T1> inline SpMat<eT>::SpMat(const SpSubview_col_list<eT, T1>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> template<typename T1> inline
SpMat<eT>& SpMat<eT>::operator=(const SpSubview_col_list<eT, T1>& X) {
	arma_extra_debug_sigprint();

	const bool alias = (this == &(X.m));

	if(alias == false) { SpSubview_col_list<eT, T1>::extract(*this, X); } else {
		SpMat<eT> tmp(X);
...
template<typename eT> template<typename T1, typename T2, typename spglue_type> inline SpMat<eT>::SpMat(const mtSpGlue<eT, T1, T2, spglue_type>& X)
	: n_rows(0)
	, n_cols(0)
	, n_elem(0)
	, n_nonzero(0)
...
template<typename eT> template<typename T1, typename T2, typename spglue_type> inline
SpMat<eT>& SpMat<eT>::operator=(const mtSpGlue<eT, T1, T2, spglue_type>& X) {
	arma_extra_debug_sigprint();
...
SpSubview_row<eT> SpMat<eT>::row(const uword row_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const SpSubview_row<eT> SpMat<eT>::row(const uword row_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview_row<eT> SpMat<eT>::operator()(const uword row_num, const span& col_span) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview_row<eT> SpMat<eT>::operator()(const uword row_num, const span& col_span) const {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
SpSubview_col<eT> SpMat<eT>::col(const uword col_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_inline
const SpSubview_col<eT> SpMat<eT>::col(const uword col_num) const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
SpSubview_col<eT> SpMat<eT>::operator()(const span& row_span, const uword col_num) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpSubview_col<eT> SpMat<eT>::operator()(const span& row_span, const uword col_num) const {
	arma_extra_debug_sigprint();
...
SpSubview_col_list<eT, T1> SpMat<eT>::cols(const Base<uword, T1>& indices) {
	arma_extra_debug_sigprint();
...
template<typename eT> template<typename T1> arma_inline
const SpSubview_col_list<eT, T1> SpMat<eT>::cols(const Base<uword, T1>& indices) const {
	arma_extra_debug_sigprint();
...
void SpMat<eT>::reshape(const uword in_rows, const uword in_cols) {
	...
	if(in_cols == 1) { (*this).reshape_helper_intovec(); } else { (*this).reshape_helper_generic(in_rows, in_cols); }
}
...
template<typename eT> inline
void SpMat<eT>::reshape_helper_generic(const uword in_rows, const uword in_cols) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::reshape_helper_intovec() {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
const SpMat<eT>& SpMat<eT>::clean(const typename get_pod_type<eT>::result threshold) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::save(const csv_name& spec, const file_type type, const bool print_status) const {
	arma_extra_debug_sigprint();
	...
	} else { save_okay = diskio::save_csv_ascii(*this, spec.filename, spec.header_ro, with_header); }

	if((print_status == true) && (save_okay == false)) { arma_debug_warn("SpMat::save(): couldn't write to ", spec.filename); }
...
template<typename eT> inline
arma_cold
bool SpMat<eT>::load(const csv_name& spec, const file_type type, const bool print_status) {
	arma_extra_debug_sigprint();
	...
	} else { load_okay = diskio::load_csv_ascii(*this, spec.filename, err_msg, spec.header_rw, with_header); }

	if(print_status == true) {
		if(load_okay == false) { if(err_msg.length() > 0) { arma_debug_warn("SpMat::load(): ", err_msg, spec.filename); } else { arma_debug_warn("SpMat::load(): couldn't read ", spec.filename); } } else {
			const uword load_n_cols = (do_trans) ? (*this).n_rows : (*this).n_cols;

			if(with_header && (spec.header_rw.n_elem != load_n_cols)) { arma_debug_warn("SpMat::load(): size mistmach between header and matrix"); }
		}
	}
...
void SpMat<eT>::steal_mem(SpMat<eT>& x) {
	...
	if((*this).vec_state == x.vec_state) { layout_ok = true; } else {
...
template<typename eT> template<typename T1, typename Functor> inline
...
template<typename eT> template<typename eT2, typename T1, typename Functor> inline
...
template<typename eT> arma_inline
arma_warn_unused
SpMat_MapMat_val<eT> SpMat<eT>::front() {
...
template<typename eT> arma_inline
arma_warn_unused
SpMat_MapMat_val<eT> SpMat<eT>::back() {
...
arma_hot
arma_warn_unused
const eT* SpMat<eT>::find_value_csc(const uword in_row, const uword in_col) const {
...
arma_hot
arma_warn_unused
bool SpMat<eT>::try_set_value_csc(const uword in_row, const uword in_col, const eT in_val) {
	const eT* val_ptr = find_value_csc(in_row, in_col);
...
arma_hot
arma_warn_unused
bool SpMat<eT>::try_add_value_csc(const uword in_row, const uword in_col, const eT in_val) {
	const eT* val_ptr = find_value_csc(in_row, in_col);
...
arma_hot
arma_warn_unused
bool SpMat<eT>::try_sub_value_csc(const uword in_row, const uword in_col, const eT in_val) {
	const eT* val_ptr = find_value_csc(in_row, in_col);
...
arma_hot
arma_warn_unused
bool SpMat<eT>::try_mul_value_csc(const uword in_row, const uword in_col, const eT in_val) {
	const eT* val_ptr = find_value_csc(in_row, in_col);
...
arma_hot
arma_warn_unused
bool SpMat<eT>::try_div_value_csc(const uword in_row, const uword in_col, const eT in_val) {
	const eT* val_ptr = find_value_csc(in_row, in_col);
...
template<typename eT> inline
void SpMat<eT>::sync_cache_simple() const {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
void SpMat<eT>::sync_csc_simple() const {
	arma_extra_debug_sigprint();
