...
<
	...
	const mtOp<uword, T1, op_find_simple>>::result find(const T1& X) {
...
template<typename T1> arma_warn_unused
inline
Col<uword> find(const SpBase<typename T1::elem_type, T1>& X, const uword k = 0) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
Col<uword> find(const SpBase<typename T1::elem_type, T1>& X, const uword k, const char* direction) {
	arma_extra_debug_sigprint();
...
<
	is_arma_type<T1>::value,
	const mtOp<uword, T1, op_find_finite>>::result find_finite(const T1& X) {
	arma_extra_debug_sigprint();
...
<
	is_arma_type<T1>::value,
	const mtOp<uword, T1, op_find_nonfinite>>::result find_nonfinite(const T1& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
Col<uword> find_finite(const SpBase<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> arma_warn_unused
inline
Col<uword> find_nonfinite(const SpBase<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
