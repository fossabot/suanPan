...
void internal_regspace_default_delta
(
	Mat<eT>& x,
...
typename enable_if2<(is_signed<sT>::value == true), void>::result internal_regspace_var_delta
(
	Mat<eT>& x,
	const typename Mat<eT>::pod_type start,
	const sT delta,
...
typename enable_if2<(is_signed<uT>::value == false), void>::result internal_regspace_var_delta
(
	Mat<eT>& x,
	const typename Mat<eT>::pod_type start,
	const uT delta,
...
typename enable_if2<is_Mat<vec_type>::value && (is_signed<sT>::value == true), vec_type>::result regspace
(
...
template<typename vec_type, typename uT> inline
typename enable_if2<is_Mat<vec_type>::value && (is_signed<uT>::value == false), vec_type>::result regspace
(
	const typename vec_type::pod_type start,
	const uT delta,
	const typename vec_type::pod_type end
...
template<typename vec_type> arma_warn_unused
inline
typename
enable_if2
<
...
inline
vec regspace(const double start, const double delta, const double end) {
	arma_extra_debug_sigprint();
...
inline
vec regspace(const double start, const double end) {
	arma_extra_debug_sigprint();
