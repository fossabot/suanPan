...
template<typename eT> arma_hot
inline
typename arma_cx_only<eT>::result op_dot::direct_dot_arma(const uword n_elem, const eT* const A, const eT* const B) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_hot
inline
typename arma_integral_only<eT>::result op_dot::direct_dot(const uword n_elem, const eT* const A, const eT* const B) { return op_dot::direct_dot_arma(n_elem, A, B); }

//! for three arrays
template<typename eT> arma_hot
inline
eT op_dot::direct_dot(const uword n_elem, const eT* const A, const eT* const B, const eT* C) {
	arma_extra_debug_sigprint();
...
inline
typename T1::elem_type op_dot::apply(const T1& X, const T2& Y) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_hot
inline
typename arma_not_cx<typename T1::elem_type>::result op_dot::apply_proxy(const Proxy<T1>& PA, const Proxy<T2>& PB) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_hot
inline
typename arma_cx_only<typename T1::elem_type>::result op_dot::apply_proxy(const Proxy<T1>& PA, const Proxy<T2>& PB) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_hot
inline
typename T1::elem_type op_norm_dot::apply(const T1& X, const T2& Y) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_hot
inline
eT op_cdot::direct_cdot_arma(const uword n_elem, const eT* const A, const eT* const B) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_hot
inline
typename T1::elem_type op_cdot::apply_unwrap(const T1& X, const T2& Y) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_hot
inline
typename T1::elem_type op_cdot::apply_proxy(const T1& X, const T2& Y) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_hot
inline
typename promote_type<typename T1::elem_type, typename T2::elem_type>::result op_dot_mixed::apply(const T1& A, const T2& B) {
	arma_extra_debug_sigprint();
	...
	for(uword i = 0; i < N; ++i) { acc += upgrade_val<in_eT1, in_eT2>::apply(PA[i]) * upgrade_val<in_eT1, in_eT2>::apply(PB[i]); }
