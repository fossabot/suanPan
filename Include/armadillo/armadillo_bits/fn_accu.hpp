...
template<typename T1, typename T2> arma_warn_unused
inline
uword accu(const mtGlue<uword, T1, T2, glue_rel_noteq>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1, typename T2> arma_warn_unused
inline
uword accu(const mtGlue<uword, T1, T2, glue_rel_eq>& X) {
	arma_extra_debug_sigprint();
...
inline
typename arma_scalar_only<T>::result accu(const T& x) { return x; }
...
template<typename T1, typename spop_type> arma_warn_unused
inline
typename T1::elem_type accu(const SpOp<T1, spop_type>& expr) {
	arma_extra_debug_sigprint();
