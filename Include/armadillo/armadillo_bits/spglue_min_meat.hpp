...
void spglue_min::apply(SpMat<typename T1::elem_type>& out, const SpGlue<T1, T2, spglue_min>& X) {
...
void spglue_min::apply_noalias(SpMat<eT>& out, const SpProxy<T1>& pa, const SpProxy<T2>& pb) {
	arma_extra_debug_sigprint();
...
void spglue_min::apply_noalias(SpMat<eT>& out, const SpMat<eT>& A, const SpMat<eT>& B) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1, typename T2> inline
void spglue_min::dense_sparse_min(Mat<eT>& out, const Base<eT, T1>& X, const SpBase<eT, T2>& Y) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
typename enable_if2<is_cx<eT>::no, eT>::result spglue_min::elem_min(const eT& a, const eT& b) { return (std::min)(a, b); }

// min of complex elements
template<typename eT> inline
typename enable_if2<is_cx<eT>::yes, eT>::result spglue_min::elem_min(const eT& a, const eT& b) { return (std::abs(a) < std::abs(b)) ? a : b; }
