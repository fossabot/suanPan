...
template<typename eT, bool do_conj> inline xtrans_mat<eT, do_conj>::xtrans_mat(const Mat<eT>& in_X)
	: X(in_X)
...
void xtrans_mat<eT, do_conj>::extract(Mat<eT>& out) const {
	arma_extra_debug_sigprint();
...
template<typename eT, bool do_conj> inline
eT xtrans_mat<eT, do_conj>::at_alt(const uword ii) const { return (*this).operator[](ii); }

template<typename eT, bool do_conj> arma_inline
eT xtrans_mat<eT, do_conj>::at(const uword in_row, const uword in_col) const {
