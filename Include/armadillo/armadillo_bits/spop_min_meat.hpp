...
void spop_min::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_min>& in) {
...
(
	...
	} else if(dim == 1) // find the minimum in each row
	{
		...
		for(uword row = 0; row < p_n_rows; ++row) { if(count[row] < p_n_cols) { value[row] = (std::min)(value[row], eT(0)); } }
...
template<typename T1> inline
typename T1::elem_type spop_min::vector_min
(
	const T1& x,
	const typename arma_not_cx<typename T1::elem_type>::result* junk
) {
...
template<typename T1> inline
typename arma_not_cx<typename T1::elem_type>::result spop_min::min(const SpBase<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename arma_not_cx<typename T1::elem_type>::result spop_min::min_with_index(const SpProxy<T1>& P, uword& index_of_min_val) {
	arma_extra_debug_sigprint();
...
(
	...
	} else if(dim == 1) // find the minimum in each row
	{
		...
		for(uword row = 0; row < p_n_rows; ++row) { if(count[row] < p_n_cols) { if(T(0) < absval[row]) { rawval[row] = eT(0); } } }
...
template<typename T1> inline
typename arma_cx_only<typename T1::elem_type>::result spop_min::min(const SpBase<typename T1::elem_type, T1>& X) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
typename arma_cx_only<typename T1::elem_type>::result spop_min::min_with_index(const SpProxy<T1>& P, uword& index_of_min_val) {
	arma_extra_debug_sigprint();
