...
void spop_strans::apply_noalias(SpMat<eT>& B, const SpMat<eT>& A) {
	...
	for(uword i = 0; i < n; ++i) { for(uword j = ia[i]; j < ia[i + 1]; ++j) { ib[ja[j] + 1]++; } }

	for(uword i = 0; i < m; ++i) { ib[i + 1] += ib[i]; }
	...
	for(uword i = m - 1; i >= 1; --i) { ib[i] = ib[i - 1]; }

	ib[0] = 0;
}

template<typename T1> inline
void spop_strans::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_strans>& in) {
	arma_extra_debug_sigprint();
...
template<typename T1> inline
void spop_strans::apply(SpMat<typename T1::elem_type>& out, const SpOp<T1, spop_htrans>& in) {
	arma_extra_debug_sigprint();
