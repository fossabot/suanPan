...
<
	...
	const mtOp<typename std::complex<typename T1::pod_type>, T1, op_cx_scalar_times>>::result operator*
(
	const T1& X,
...
<
	...
	const Glue<T1, Op<T2, op_diagmat>, glue_times_diag>>::result operator*
...
<
	...
	const Glue<Op<T1, op_diagmat>, T2, glue_times_diag>>::result operator*
...
<
	...
	const Glue<T1, T2, glue_times>>::result operator*
...
<
	...
	const mtGlue<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result, T1, T2, glue_mixed_times>>::result operator*
(
	const T1& X,
	const T2& Y
) {
...
<
	...
	const mtSpOp<typename std::complex<typename T1::pod_type>, T1, spop_cx_scalar_times>>::result operator*
(
	const T1& X,
	const std::complex<typename T1::pod_type>& k
) {
...
<
	...
	const SpGlue<T1, T2, spglue_times>>::result operator*
(
	const T1& x,
	const T2& y
) {
...
<
	...
	const mtSpGlue<typename promote_type<typename T1::elem_type, typename T2::elem_type>::result, T1, T2, spglue_times_mixed>>::result operator*
(
	const T1& X,
	const T2& Y
) {
