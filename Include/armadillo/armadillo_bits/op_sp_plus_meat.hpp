...
void op_sp_plus::apply(Mat<typename T1::elem_type>& out, const SpToDOp<T1, op_sp_plus>& in) {
...
template<typename T1> inline
void op_sp_plus::apply(SpMat<typename T1::elem_type>& out, const SpToDOp<T1, op_sp_plus>& in) {
	arma_extra_debug_sigprint();
...
void op_sp_plus::apply_inside_schur(SpMat<eT>& out, const T2& x, const SpToDOp<T3, op_sp_plus>& y) {
	arma_extra_debug_sigprint();
...
// used for the optimization of sparse / (sparse + scalar)
template<typename eT, typename T2, typename T3> inline
void op_sp_plus::apply_inside_div(SpMat<eT>& out, const T2& x, const SpToDOp<T3, op_sp_plus>& y) {
	arma_extra_debug_sigprint();
