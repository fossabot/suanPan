...
namespace newarp {
	...
	void DoubleShiftQR<eT>::compute_reflector(const eT& x1, const eT& x2, const eT& x3, uword ind) {
	...
	void DoubleShiftQR<eT>::compute_reflector(const eT* x, uword ind) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	void DoubleShiftQR<eT>::update_block(uword il, uword iu) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	void DoubleShiftQR<eT>::apply_PX(Mat<eT>& X, uword oi, uword oj, uword nrow, uword ncol, uword u_ind) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	void DoubleShiftQR<eT>::apply_PX(eT* x, uword u_ind) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	void DoubleShiftQR<eT>::apply_XP(Mat<eT>& X, uword oi, uword oj, uword nrow, uword ncol, uword u_ind) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline DoubleShiftQR<eT>::DoubleShiftQR(uword size)
		: n(size)
		, prec(std::numeric_limits<eT>::epsilon())
		, eps_rel(prec)
		, eps_abs(prec)
		, computed(false) { arma_extra_debug_sigprint(); }

	template<typename eT> inline DoubleShiftQR<eT>::DoubleShiftQR(const Mat<eT>& mat_obj, eT s, eT t)
		: n(mat_obj.n_rows)
		, mat_H(n, n)
		, shift_s(s)
		, shift_t(t)
	...
	template<typename eT> inline
	void DoubleShiftQR<eT>::apply_QtY(Col<eT>& y) {
		arma_extra_debug_sigprint();
	...
	template<typename eT> inline
	void DoubleShiftQR<eT>::apply_YQ(Mat<eT>& Y) {
		arma_extra_debug_sigprint();
