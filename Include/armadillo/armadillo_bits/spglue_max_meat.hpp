...
void spglue_max::apply(SpMat<typename T1::elem_type>& out, const SpGlue<T1, T2, spglue_max>& X) {
	...
	if(is_alias == false) { spglue_max::apply_noalias(out, pa, pb); } else {
		SpMat<eT> tmp;
...
void spglue_max::apply_noalias(SpMat<eT>& out, const SpProxy<T1>& pa, const SpProxy<T2>& pb) {
	arma_extra_debug_sigprint();
...
void spglue_max::apply_noalias(SpMat<eT>& out, const SpMat<eT>& A, const SpMat<eT>& B) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1, typename T2> inline
void spglue_max::dense_sparse_max(Mat<eT>& out, const Base<eT, T1>& X, const SpBase<eT, T2>& Y) {
	arma_extra_debug_sigprint();
	...
	for(uword c = 0; c < n_cols; ++c)
		for(uword r = 0; r < n_rows; ++r) { out.at(r, c) = elem_max(pa.at(r, c), pb.at(r, c)); }
}

//! max of non-complex elements
template<typename eT> inline
typename enable_if2<is_cx<eT>::no, eT>::result spglue_max::elem_max(const eT& a, const eT& b) { return (std::max)(a, b); }

//! max of complex elements
template<typename eT> inline
typename enable_if2<is_cx<eT>::yes, eT>::result spglue_max::elem_max(const eT& a, const eT& b) { return (std::abs(a) > std::abs(b)) ? a : b; }
