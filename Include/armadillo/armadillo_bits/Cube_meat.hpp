...
void Cube<eT>::delete_mat() {
	...
	if((n_slices > 0) && (mat_ptrs != nullptr)) {
		for(uword uslice = 0; uslice < n_slices; ++uslice) { if(mat_ptrs[uslice] != nullptr) { delete access::rw(mat_ptrs[uslice]); } }
...
void Cube<eT>::create_mat() {
	...
	if(n_slices == 0) { access::rw(mat_ptrs) = nullptr; } else {
		if(mem_state <= 2) {
		...
		for(uword uslice = 0; uslice < n_slices; ++uslice) { mat_ptrs[uslice] = nullptr; }
	}
}
...
//! remove specified slices
template<typename eT> template<typename T1> inline
void Cube<eT>::shed_slices(const Base<uword, T1>& indices) {
	arma_extra_debug_sigprint();
	...
	if(arma_config::debug) { for(uword i = 0; i < N; ++i) { arma_debug_check((slices_to_shed_mem[i] >= n_slices), "Cube::shed_slices(): indices out of bounds"); } }
...
template<typename eT> inline
const Cube<eT>& Cube<eT>::clean(const typename get_pod_type<eT>::result threshold) {
	arma_extra_debug_sigprint();
