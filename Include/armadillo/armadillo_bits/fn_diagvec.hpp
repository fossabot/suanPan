...
arma_inline
const Op<T1, op_diagvec> diagvec(const Base<typename T1::elem_type, T1>& X) {
...
template<typename T1> arma_warn_unused
arma_inline
const Op<T1, op_diagvec2> diagvec(const Base<typename T1::elem_type, T1>& X, const sword diag_id) {
	arma_extra_debug_sigprint();
