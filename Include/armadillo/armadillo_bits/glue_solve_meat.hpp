...
bool glue_solve_gen::apply(Mat<eT>& out, const Base<eT, T1>& A_expr, const Base<eT, T2>& B_expr, const uword flags) {
	...
	if(A.n_rows == A.n_cols) {
      ...
      const bool try_sympd = false;
		...
		if((status == true) && (rcond > T(0)) && (rcond < auxlib::epsilon_lapack(A))) { arma_debug_warn("solve(): solution computed, but system seems singular to working precision (rcond: ", rcond, ")"); }
...
template<typename T1, typename T2> inline
void glue_solve_tri_default::apply(Mat<typename T1::elem_type>& out, const Glue<T1, T2, glue_solve_tri_default>& X) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1, typename T2> inline
bool glue_solve_tri_default::apply(Mat<eT>& actual_out, const Base<eT, T1>& A_expr, const Base<eT, T2>& B_expr, const uword flags) {
	arma_extra_debug_sigprint();
...
template<typename eT, typename T1, typename T2> inline
bool glue_solve_tri::apply(Mat<eT>& actual_out, const Base<eT, T1>& A_expr, const Base<eT, T2>& B_expr, const uword flags) {
	arma_extra_debug_sigprint();
