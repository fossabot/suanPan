...
bool sp_auxlib::eigs_sym(Col<eT>& eigval, Mat<eT>& eigvec, const SpBase<eT, T1>& X, const uword n_eigvals, const form_type form_val, const eT sigma, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
bool sp_auxlib::eigs_sym_newarp(Col<eT>& eigval, Mat<eT>& eigvec, const SpMat<eT>& X, const uword n_eigvals, const form_type form_val, const eT sigma, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
template<typename eT> inline
bool sp_auxlib::eigs_sym_arpack(Col<eT>& eigval, Mat<eT>& eigvec, const SpMat<eT>& X, const uword n_eigvals, const form_type form_val, const eT sigma, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
#if defined(ARMA_USE_ARPACK)
	{
		...
		if(opts.subdim != 0) {
			...
			} else if(blas_int(opts.subdim) > n) {
				arma_debug_warn("eigs_sym(): opts.subdim cannot be greater than n_rows; using n_rows instead of ", opts.subdim);
				ncv = n;
			} else { ncv = blas_int(opts.subdim); }
		}
...
bool sp_auxlib::eigs_gen(Col<std::complex<T>>& eigval, Mat<std::complex<T>>& eigvec, const SpBase<T, T1>& X, const uword n_eigvals, const form_type form_val, const std::complex<T> sigma, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
bool sp_auxlib::eigs_gen_newarp(Col<std::complex<T>>& eigval, Mat<std::complex<T>>& eigvec, const SpMat<T>& X, const uword n_eigvals, const form_type form_val, const std::complex<T> sigma, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
template<typename T> inline
bool sp_auxlib::eigs_gen_arpack(Col<std::complex<T>>& eigval, Mat<std::complex<T>>& eigvec, const SpMat<T>& X, const uword n_eigvals, const form_type form_val, const std::complex<T> sigma, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
template<typename T, typename T1> inline
bool sp_auxlib::eigs_gen(Col<std::complex<T>>& eigval, Mat<std::complex<T>>& eigvec, const SpBase<std::complex<T>, T1>& X_expr, const uword n_eigvals, const form_type form_val, const std::complex<T> sigma, const eigs_opts& opts) {
	arma_extra_debug_sigprint();
...
bool sp_auxlib::copy_to_supermatrix(superlu::SuperMatrix& out, const SpMat<eT>& A) {
	...
	if(is_float<eT>::value) { out.Dtype = superlu::SLU_S; } else if(is_double<eT>::value) { out.Dtype = superlu::SLU_D; } else if(is_cx_float<eT>::value) { out.Dtype = superlu::SLU_C; } else if(is_cx_double<eT>::value) { out.Dtype = superlu::SLU_Z; }
...
void sp_auxlib::destroy_supermatrix(superlu::SuperMatrix& out) {
	...
	// Clean up.
	if(out.Stype == superlu::SLU_NC) { superlu::destroy_compcol_mat(&out); } else if(out.Stype == superlu::SLU_NCP) { superlu::destroy_compcolperm_mat(&out); } else if(out.Stype == superlu::SLU_DN) {
...
template<typename eT, typename T> inline
...
(
...
#if defined(ARMA_USE_ARPACK)
	{
		...
		} else { status_x = sp_auxlib::copy_to_supermatrix(x.get_ref(), X); }
...
template<typename eT> inline
bool sp_auxlib::rudimentary_sym_check(const SpMat<eT>& X) {
	arma_extra_debug_sigprint();
...
template<typename T> inline
bool sp_auxlib::rudimentary_sym_check(const SpMat<std::complex<T>>& X) {
	arma_extra_debug_sigprint();
...
inline supermatrix_wrangler::~supermatrix_wrangler() {
...
inline
superlu::SuperMatrix& supermatrix_wrangler::get_ref() {
	used = true;
...
inline
superlu::SuperMatrix* supermatrix_wrangler::get_ptr() {
	used = true;
...
inline superlustat_wrangler::~superlustat_wrangler() {
	arma_extra_debug_sigprint_this(this);
...
inline
superlu::SuperLUStat_t* superlustat_wrangler::get_ptr() { return &stat; }

//

inline superluintarray_wrangler::~superluintarray_wrangler() {
	arma_extra_debug_sigprint_this(this);
...
inline superluintarray_wrangler::superluintarray_wrangler(const uword n_elem) {
	arma_extra_debug_sigprint_this(this);
...
inline
int* superluintarray_wrangler::get_ptr() { return mem; }
