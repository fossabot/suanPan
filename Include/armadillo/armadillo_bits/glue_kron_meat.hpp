...
void glue_kron::direct_kron(Mat<eT>& out, const Mat<eT>& A, const Mat<eT>& B) {
	...
	for(uword j = 0; j < A_cols; j++) { for(uword i = 0; i < A_rows; i++) { out.submat(i * B_rows, j * B_cols, (i + 1) * B_rows - 1, (j + 1) * B_cols - 1) = A.at(i, j) * B; } }
}
...
void glue_kron::direct_kron(Mat<std::complex<T>>& out, const Mat<std::complex<T>>& A, const Mat<T>& B) {
	arma_extra_debug_sigprint();
	...
	for(uword j = 0; j < A_cols; j++) { for(uword i = 0; i < A_rows; i++) { out.submat(i * B_rows, j * B_cols, (i + 1) * B_rows - 1, (j + 1) * B_cols - 1) = A.at(i, j) * tmp_B; } }
}
...
template<typename T> inline
void glue_kron::direct_kron(Mat<std::complex<T>>& out, const Mat<T>& A, const Mat<std::complex<T>>& B) {
	arma_extra_debug_sigprint();
...
void glue_kron::apply(Mat<typename T1::elem_type>& out, const Glue<T1, T2, glue_kron>& X) {
	arma_extra_debug_sigprint();
