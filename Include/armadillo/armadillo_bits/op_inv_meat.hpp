...
void op_inv::apply(Mat<typename T1::elem_type>& out, const Op<T1, op_inv>& X) {
	...
	if(strip.do_diagmat) { status = op_inv::apply_diagmat(out, strip.M); } else {
		...
		if(U.is_alias(out)) {
			...
			out.steal_mem(tmp);
		} else { status = op_inv::apply_noalias(out, U.M); }
	}
...
bool op_inv::apply_noalias(Mat<eT>& out, const Mat<eT>& A) {
	arma_extra_debug_sigprint();
	...
	if(A.n_rows <= 4) { status = auxlib::inv_tiny(out, A); } else if(A.is_diagmat()) { return op_inv::apply_diagmat(out, A); } else {
...
template<typename T1> inline
bool op_inv::apply_diagmat(Mat<typename T1::elem_type>& out, const T1& X) {
	arma_extra_debug_sigprint();
