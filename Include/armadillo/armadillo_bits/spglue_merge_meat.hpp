...
inline
void spglue_merge::subview_merge(SpSubview<eT>& sv, const SpMat<eT>& B) {
...
template<typename eT> arma_hot
inline
void spglue_merge::subview_merge(SpSubview<eT>& sv, const Mat<eT>& B) {
	arma_extra_debug_sigprint();
	...
	for(uword i = 0; i < B_n_elem; ++i) { B_n_nonzero += (B_memptr[i] != eT(0)) ? uword(1) : uword(0); }
...
template<typename eT> arma_hot
inline
void spglue_merge::symmat_merge(SpMat<eT>& out, const SpMat<eT>& A, const SpMat<eT>& B) {
	arma_extra_debug_sigprint();
...
template<typename eT> arma_hot
inline
void spglue_merge::diagview_merge(SpMat<eT>& out, const SpMat<eT>& A, const SpMat<eT>& B) {
	arma_extra_debug_sigprint();
