...
void T2D2S::initialize(const shared_ptr<DomainBase>& D) {
	...
	const auto linear_density = t_section->get_parameter(ParameterType::LINEARDENSITY);
	if(linear_density > 0.) trial_mass = current_mass = initial_mass = t_trans->to_global_mass_mat(linear_density);
}
...
void T2D2S::GetData(vtkSmartPointer<vtkDoubleArray>& arrays, const OutputType type) {
	mat t_disp(6, t_node, fill::zeros);

	if(OutputType::A == type) t_disp.rows(0, 1) = reshape(get_current_acceleration(), t_dof, t_node);
	else if(OutputType::V == type) t_disp.rows(0, 1) = reshape(get_current_velocity(), t_dof, t_node);
	else if(OutputType::U == type) t_disp.rows(0, 1) = reshape(get_current_displacement(), t_dof, t_node);

	for(unsigned I = 0; I < t_node; ++I) arrays->SetTuple(node_encoding(I), t_disp.colptr(I));
}

void T2D2S::SetDeformation(vtkSmartPointer<vtkPoints>& nodes, const double amplifier) {
