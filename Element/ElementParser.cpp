...
void new_b21(unique_ptr<Element>& return_obj, istringstream& command) {
	...
	if(!get_optional_input(command, nonlinear)) suanpan_error("new_b21() needs a valid nonlinear geomtery switch (0,1).\n");
...
void new_b21e(unique_ptr<Element>& return_obj, istringstream& command, const unsigned which) {
	...
	string nonlinear = "false";
	if(!get_optional_input(command, nonlinear)) suanpan_error("new_b21e() needs a valid nonlinear geomtery switch (0,1).\n");
...
void new_b21h(unique_ptr<Element>& return_obj, istringstream& command) {
	...
	string nonlinear = "false";
	if(!get_optional_input(command, nonlinear)) suanpan_error("new_b21h() needs a valid nonlinear geomtery switch (0,1).\n");
...
void new_c3d20(unique_ptr<Element>& return_obj, istringstream& command) {
	...
	else if(!get_input(command, nonlinear)) suanpan_error("new_c3d20() needs a valid nonlinear geomtery switch (0,1).\n");
...
void new_c3d4(unique_ptr<Element>& return_obj, istringstream& command) {
	...
	else if(!get_input(command, nonlinear)) suanpan_error("new_c3d4() needs a valid nonlinear geomtery switch (0,1).\n");
...
void new_c3d8(unique_ptr<Element>& return_obj, istringstream& command) {
	...
	else if(!get_input(command, nonlinear)) suanpan_error("new_c3d8() needs a valid nonlinear geomtery switch (0,1).\n");
...
void new_cp4r(unique_ptr<Element>& return_obj, istringstream& command) {
	...
	auto thickness = 1.;
	if(!get_optional_input(command, thickness)) suanpan_error("new_cp4r() needs a valid thickness.\n");
...
void new_cp6(unique_ptr<Element>& return_obj, istringstream& command) {
	...
	auto thickness = 1.;
	if(!get_optional_input(command, thickness)) suanpan_error("new_cp6() needs a valid thickness.\n");

	string nonlinear = "false";
	if(!get_optional_input(command, nonlinear)) suanpan_error("new_cp6() needs a valid nonlinear geometry switch.\n");
...
void new_dcp3(unique_ptr<Element>& return_obj, istringstream& command) {
	...
	auto thickness = 1.;
	if(!get_optional_input(command, thickness)) suanpan_debug("new_dcp3() assumes thickness to be unit.\n");
...
void new_dcp4(unique_ptr<Element>& return_obj, istringstream& command) {
	...
	auto thickness = 1.;
	if(!get_optional_input(command, thickness)) suanpan_debug("new_dcp4() assumes thickness to be unit.\n");
...
void new_eb21(unique_ptr<Element>& return_obj, istringstream& command) {
	...
	else if(!get_input(command, nonlinear)) suanpan_error("new_eb21() needs a valid nonlinear geomtery switch (0,1).\n");
...
void new_f21(unique_ptr<Element>& return_obj, istringstream& command) {
	...
	else if(!get_input(command, nonlinear)) suanpan_error("new_f21() needs a valid nonlinear geomtery switch (0,1).\n");
...
void new_f21h(unique_ptr<Element>& return_obj, istringstream& command) {
	...
	else if(!get_input(command, nonlinear)) suanpan_error("new_f21h() needs a valid nonlinear geomtery switch (0,1).\n");
...
void new_rcp4(unique_ptr<Element>& return_obj, istringstream& command) {
	unsigned tag;
	if(!get_input(command, tag)) {
		suanpan_error("new_rcp4() needs a tag.\n");
		return;
	}
	...
	for(auto& I : node_tag)
		if(!get_input(command, I)) {
			suanpan_error("new_rcp4() needs four valid nodes.\n");
			return;
		}

	unsigned material_tag;
	if(!get_input(command, material_tag)) {
		suanpan_error("new_rcp4() needs a valid material tag.\n");
		return;
	}

	auto thickness = 1.;
	if(!command.eof() && !get_input(command, thickness)) {
		suanpan_error("new_rcp4() needs a valid thickness.\n");
		return;
	}

	auto rho_x = 0., rho_y = 0.;
	auto mat_tag_x = 0, mat_tag_y = 0;
	if(!command.eof() && !get_input(command, rho_x)) suanpan_error("new_rcp4() needs a valid rho_x.\n");
	if(!command.eof() && !get_input(command, mat_tag_x)) suanpan_error("new_rcp4() needs a valid mat_x.\n");
	if(!command.eof() && !get_input(command, rho_y)) suanpan_error("new_rcp4() needs a valid rho_y.\n");
	if(!command.eof() && !get_input(command, mat_tag_y)) suanpan_error("new_rcp4() needs a valid mat_y.\n");
...
void new_rebarlayer(unique_ptr<Element>& return_obj, istringstream& command) {
	unsigned tag;
	if(!get_input(command, tag)) {
		suanpan_error("new_rebarlayer() needs a valid tag.\n");
		return;
	}
	...
	for(auto& I : node_tag)
		if(!get_input(command, I)) {
			suanpan_error("new_rebarlayer() needs four valid nodes.\n");
			return;
		}
	...
	for(auto& I : configuration)
		if(!get_input(command, I)) {
			suanpan_error("new_rebarlayer() needs nine valid paramters.\n");
			return;
		}

	auto thickness = 1.;
	if(command.eof()) suanpan_debug("new_rebarlayer() assumes thickness to be unit.\n");
	else if(!get_input(command, thickness)) suanpan_error("new_rebarlayer() needs a valid thickness.\n");
...
void new_rgcmq(unique_ptr<Element>& return_obj, istringstream& command) {
	unsigned tag;
	if(!get_input(command, tag)) {
		suanpan_error("new_rgcmq() needs a valid tag.\n");
		return;
	}
	...
	for(auto& I : node_tag)
		if(!get_input(command, I)) {
			suanpan_error("new_rgcmq() needs four valid nodes.\n");
			return;
		}

	unsigned material_tag;
	if(!get_input(command, material_tag)) {
		suanpan_error("new_rgcmq() needs a valid material tag.\n");
		return;
	}

	auto thickness = 1.;
	if(command.eof()) suanpan_debug("new_rgcmq() assumes thickness to be unit.\n");
	else if(!get_input(command, thickness)) suanpan_error("new_rgcmq() needs a valid thickness.\n");

	string reduced = "I";
	if(!command.eof() && !get_input(command, reduced)) suanpan_error("new_rgcmq() needs a valid reduced scheme switch.\n");

	auto rho_x = 0., rho_y = 0.;
	auto mat_tag_x = 0, mat_tag_y = 0;
	if(!command.eof() && !get_input(command, rho_x)) suanpan_error("new_rgcmq() needs a valid rho_x.\n");
	if(!command.eof() && !get_input(command, mat_tag_x)) suanpan_error("new_rgcmq() needs a valid mat_x.\n");
	if(!command.eof() && !get_input(command, rho_y)) suanpan_error("new_rgcmq() needs a valid rho_y.\n");
	if(!command.eof() && !get_input(command, mat_tag_y)) suanpan_error("new_rgcmq() needs a valid mat_y.\n");
...
void new_srgcmqi(unique_ptr<Element>& return_obj, istringstream& command) {
	unsigned tag;
	if(!get_input(command, tag)) {
		suanpan_error("new_srgcmqi() needs a valid tag.\n");
		return;
	}
	...
	for(auto& I : node_tag)
		if(!get_input(command, I)) {
			suanpan_error("new_srgcmqi() needs four valid nodes.\n");
			return;
		}

	unsigned material_tag;
	if(!get_input(command, material_tag)) {
		suanpan_error("new_srgcmqi() needs a valid material tag.\n");
		return;
	}

	auto thickness = 1.;
	if(command.eof()) suanpan_debug("new_srgcmqi() assumes thickness to be unit.\n");
	else if(!get_input(command, thickness)) suanpan_error("new_srgcmqi() needs a valid thickness.\n");

	auto rho_x = 0., rho_y = 0.;
	auto mat_tag_x = 0, mat_tag_y = 0;
	if(!command.eof() && !get_input(command, rho_x)) suanpan_error("new_srgcmqi() needs a valid rho_x.\n");
	if(!command.eof() && !get_input(command, mat_tag_x)) suanpan_error("new_srgcmqi() needs a valid mat_x.\n");
	if(!command.eof() && !get_input(command, rho_y)) suanpan_error("new_srgcmqi() needs a valid rho_y.\n");
	if(!command.eof() && !get_input(command, mat_tag_y)) suanpan_error("new_srgcmqi() needs a valid mat_y.\n");
...
void new_srgcmql(unique_ptr<Element>& return_obj, istringstream& command) {
	unsigned tag;
	if(!get_input(command, tag)) {
		suanpan_error("new_srgcmql() needs a valid tag.\n");
		return;
	}
	...
	for(auto& I : node_tag)
		if(!get_input(command, I)) {
			suanpan_error("new_srgcmql() needs four valid nodes.\n");
			return;
		}

	unsigned material_tag;
	if(!get_input(command, material_tag)) {
		suanpan_error("new_srgcmql() needs a valid material tag.\n");
		return;
	}

	auto thickness = 1.;
	if(command.eof()) suanpan_debug("new_srgcmql() assumes thickness to be unit.\n");
	else if(!get_input(command, thickness)) suanpan_error("new_srgcmql() needs a valid thickness.\n");

	auto rho_x = 0., rho_y = 0.;
	auto mat_tag_x = 0, mat_tag_y = 0;
	if(!command.eof() && !get_input(command, rho_x)) suanpan_error("new_srgcmql() needs a valid rho_x.\n");
	if(!command.eof() && !get_input(command, mat_tag_x)) suanpan_error("new_srgcmql() needs a valid mat_x.\n");
	if(!command.eof() && !get_input(command, rho_y)) suanpan_error("new_srgcmql() needs a valid rho_y.\n");
	if(!command.eof() && !get_input(command, mat_tag_y)) suanpan_error("new_srgcmql() needs a valid mat_y.\n");
...
void new_srgcmqg(unique_ptr<Element>& return_obj, istringstream& command) {
	unsigned tag;
	if(!get_input(command, tag)) {
		suanpan_error("new_srgcmqg() needs a valid tag.\n");
		return;
	}
	...
	for(auto& I : node_tag)
		if(!get_input(command, I)) {
			suanpan_error("new_srgcmqg() needs four valid nodes.\n");
			return;
		}

	unsigned material_tag;
	if(!get_input(command, material_tag)) {
		suanpan_error("new_srgcmqg() needs a valid material tag.\n");
		return;
	}

	auto thickness = 1.;
	if(command.eof()) suanpan_debug("new_srgcmqg() assumes thickness to be unit.\n");
	else if(!get_input(command, thickness)) suanpan_error("new_srgcmqg() needs a valid thickness.\n");

	auto rho_x = 0., rho_y = 0.;
	auto mat_tag_x = 0, mat_tag_y = 0;
	if(!command.eof() && !get_input(command, rho_x)) suanpan_error("new_srgcmqg() needs a valid rho_x.\n");
	if(!command.eof() && !get_input(command, mat_tag_x)) suanpan_error("new_srgcmqg() needs a valid mat_x.\n");
	if(!command.eof() && !get_input(command, rho_y)) suanpan_error("new_srgcmqg() needs a valid rho_y.\n");
	if(!command.eof() && !get_input(command, mat_tag_y)) suanpan_error("new_srgcmqg() needs a valid mat_y.\n");
