...
void SGCMQ::initialize(const shared_ptr<DomainBase>& D) {
	...
	if(suanpan::approx_equal(mat_proto->get_parameter(ParameterType::PLANETYPE), static_cast<double>(PlaneType::E))) suanpan::hacker(thickness) = 1.;
	...
	form_mass(mat_proto->get_parameter(), diff_coor);
...
void SGCMQ::print() {
	...
	for(auto I = 0u, J = 1u; I < static_cast<unsigned>(int_pt.size()); ++I, ++J) {
		suanpan_info("Integration Point %u:\t", J);
