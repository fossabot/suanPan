...
void Element::update_strain_energy() {
	...
	if(current_resistance.is_empty())
		...
		for(const auto& I : node_ptr) {
			auto& t_disp = I.lock()->get_incre_displacement();
			for(unsigned J = 0; J < num_dof; ++J) {
				incre_energy += t_disp(J) * (trial_resistance(idx) + current_resistance(idx));
				++idx;
			}
		}

	strain_energy += .5 * incre_energy;
}

void Element::update_kinetic_energy() {
	if(trial_mass.is_empty()) return;

	vec t_velocity(num_size);

	auto idx = 0;
	for(unsigned I = 0; I < num_node; ++I) {
		auto& t_vel = node_ptr[I].lock()->get_trial_velocity();
		for(unsigned J = 0; J < num_dof; ++J) t_velocity(idx++) = t_vel(J);
	}
...
void Element::update_viscous_energy() {
	...
	if(current_damping_force.is_empty())
		...
		for(const auto& I : node_ptr) {
			auto& t_disp = I.lock()->get_incre_displacement();
			for(unsigned J = 0; J < num_dof; ++J) {
				incre_energy += t_disp(J) * (trial_damping_force(idx) + current_damping_force(idx));
				++idx;
			}
		}

	viscous_energy += .5 * incre_energy;
}
...
void Element::update_complementary_energy() {
	...
	if(current_resistance.is_empty())
		for(const auto& I : node_ptr) {
			...
			for(unsigned J = 0; J < num_dof; ++J) incre_energy += (c_disp(J) + t_disp(J)) * trial_resistance(idx++);
		}
	else
		for(const auto& I : node_ptr) {
			auto& t_disp = I.lock()->get_trial_displacement();
			auto& c_disp = I.lock()->get_current_displacement();
			for(unsigned J = 0; J < num_dof; ++J) {
				incre_energy += (c_disp(J) + t_disp(J)) * (trial_resistance(idx) - current_resistance(idx));
				++idx;
			}
		}
...
Element::Element(const unsigned T, const unsigned NN, const unsigned ND, uvec&& NT, uvec&& MT, const bool F, const MaterialType MTP)
	: ElementData{std::forward<uvec>(NT), std::forward<uvec>(MT), uvec{}, F, true, true, true, true, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}}
	, ElementBase(T)
	, num_node(NN)
	, num_dof(ND)
	, mat_type(MTP)
...
Element::Element(const unsigned T, const unsigned NN, const unsigned ND, uvec&& NT, uvec&& ST, const bool F, const SectionType STP)
	: ElementData{std::forward<uvec>(NT), uvec{}, std::forward<uvec>(ST), F, true, true, true, true, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}}
	, ElementBase(T)
	, num_node(NN)
	, num_dof(ND)
...
Element::Element(const unsigned T, const unsigned ND, uvec&& GT)
	: ElementData{std::forward<uvec>(GT), {}, {}, false, true, true, true, true, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}}
	, ElementBase(T)
...
Element::Element(const unsigned T, const unsigned ND, const unsigned ET, const unsigned NT)
	: ElementData{{NT}, {}, {}, false, true, true, true, true, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}, {}}
	, ElementBase(T)
	, num_node(static_cast<unsigned>(-1))
	, num_dof(ND)
	, use_other(ET)
...
void Element::initialize_base(const shared_ptr<DomainBase>& D) {
	...
	if(use_group) {
		...
		counter = 0;
		for(auto& I : pool) for(auto J : *I) n_encoding(counter++) = J;
	}
	...
	for(const auto& t_tag : node_encoding) {
		if(!D->find<Node>(t_tag)) {
			suanpan_debug("Element %u cannot find node %u, now disable it.\n", get_tag(), t_tag);
			D->disable_element(get_tag());
			return;
		}
	...
	if(inactive) {
		suanpan_debug("Element %u finds inactive nodes, now disable it.\n", get_tag());
		D->disable_element(get_tag());
		return;
	}
	...
	if(MaterialType::D0 != mat_type)
		for(const auto& t_tag : material_tag)
			if(auto& t_material = D->get<Material>(t_tag); nullptr == t_material || !t_material->is_active() || t_material->get_material_type() != mat_type) {
				suanpan_debug("Element %u cannot find valid material %llu, now disable it.\n", get_tag(), t_tag);
				D->disable_element(get_tag());
				return;
			}
	...
	if(SectionType::D0 != sec_type)
		for(const auto& t_tag : section_tag)
			if(auto& t_section = D->get<Section>(t_tag); nullptr == t_section || !t_section->is_active() || t_section->get_section_type() != sec_type) {
				suanpan_debug("Element %u cannot find valid section %llu, now disable it.\n", get_tag(), t_tag);
				D->disable_element(get_tag());
				return;
			}
...
void ConstantDamping(ElementData* E) {
...
void ConstantStiffness(ElementData* E) {
...
void ConstantGeometry(ElementData* E) {
