...
int Joint::commit_status() {
	auto code = 0;
	for(auto& I : j_material) code += I->commit_status();
	return code;
}

int Joint::clear_status() {
	auto code = 0;
	for(auto& I : j_material) code += I->clear_status();
	return code;
}

int Joint::reset_status() {
	auto code = 0;
	for(auto& I : j_material) code += I->reset_status();
	return code;
}
...
vector<vec> Joint::record(const OutputType P) {
	...
	for(auto& I : j_material) for(auto& J : I->record(P)) data.emplace_back(J);
