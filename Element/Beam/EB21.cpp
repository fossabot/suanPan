...
void EB21::Setup() {
	...
	for(unsigned I = 0; I < b_node; ++I) {
		vtk_cell->GetPointIds()->SetId(I, static_cast<long long>(node_encoding(I)));
...
void EB21::GetData(vtkSmartPointer<vtkDoubleArray>& arrays, const OutputType type) {
	...
	for(unsigned I = 0; I < b_node; ++I) arrays->SetTuple(static_cast<long long>(node_encoding(I)), t_disp.colptr(I));
}
...
void EB21::SetDeformation(vtkSmartPointer<vtkPoints>& nodes, const double amplifier) {
	...
	for(unsigned I = 0; I < b_node; ++I) nodes->SetPoint(static_cast<long long>(node_encoding(I)), ele_disp(I, 0), ele_disp(I, 1), 0.);
}
