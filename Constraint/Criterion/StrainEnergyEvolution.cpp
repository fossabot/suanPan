...
void StrainEnergyEvolution::initialize(const shared_ptr<DomainBase>& D) {
	auto node_pool = D->get_node_connectivity();
	...
	suanpan_for_each(map.begin(), map.end(), [](vector<uword>& element) {
...
int StrainEnergyEvolution::process(const shared_ptr<DomainBase>& D) {
...
#else
	for(uword I = 0; I < current_energy.n_elem; ++I)
		if(D->find<Element>(I)) {
			const auto& t_element = D->get<Element>(I);
			if(!t_element->is_active()) {
				current_energy(I) = 0.;
...
#ifdef SUANPAN_MT
		tbb::parallel_for(0llu, current_energy.n_elem, [&](const uword I) {
			for(auto J : map[I]) current_energy(I) += previous_energy(J);
